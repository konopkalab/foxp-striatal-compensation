...
load("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P09/18_DOUBLETFINDER_P2CKO/NA_SEURAT_P2CKO_P9_CB_DF.RData")
# seuObjDF
...
p9.p2cko.na16.f <-  row.names(seuObjDFremoved@meta.data[seuObjDFremoved@meta.data$GenoAgeSample == "P2CKO_P09_NA16",])
p9.p2cko.na22.f <-  row.names(seuObjDFremoved@meta.data[seuObjDFremoved@meta.data$GenoAgeSample == "P2CKO_P09_NA22",])
p9.p2cko.na43.f <-  row.names(seuObjDFremoved@meta.data[seuObjDFremoved@meta.data$GenoAgeSample == "P2CKO_P09_NA43",])

## load dataset from CellBender run
p9.p2cko.na16.cb <- Read10X_h5("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P09/01_COUNTS_RAW/NA16_raw_feature_bc_matrix/CellBender_Out_filtered.h5", use.names = TRUE, unique.features = TRUE)
print(dim(p9.p2cko.na16.cb))
# 53715 11380

p9.p2cko.na22.cb <- Read10X_h5("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P09/01_COUNTS_RAW/NA22_raw_feature_bc_matrix/CellBender_Out_filtered.h5", use.names = TRUE, unique.features = TRUE)
print(dim(p9.p2cko.na22.cb))
# 53715 13680

p9.p2cko.na43.cb <- Read10X_h5("/project/Neuroinformatics_Core/Konopka_lab/akulk1/NEW_SEP2022_NK_NA_EO_CO/03_CELLBENDER_NA/NA43_CellBender_Out_filtered.h5", use.names = TRUE, unique.features = TRUE)
print(dim(p9.p2cko.na43.cb))
# 53715 37358


## adding sample & condition info to column names
colnames(p9.p2cko.na16.cb) <- paste("P2CKO_P09_NA16", colnames(p9.p2cko.na16.cb), sep = "_")
colnames(p9.p2cko.na22.cb) <- paste("P2CKO_P09_NA22", colnames(p9.p2cko.na22.cb), sep = "_")
colnames(p9.p2cko.na43.cb) <- paste("P2CKO_P09_NA43", colnames(p9.p2cko.na43.cb), sep = "_")



## fetching counts for common cellbarcodes
p9.p2cko.na16 <- p9.p2cko.na16.cb[,colnames(p9.p2cko.na16.cb) %in% p9.p2cko.na16.f]
p9.p2cko.na22 <- p9.p2cko.na22.cb[,colnames(p9.p2cko.na22.cb) %in% p9.p2cko.na22.f]
p9.p2cko.na43 <- p9.p2cko.na43.cb[,colnames(p9.p2cko.na43.cb) %in% p9.p2cko.na43.f]
...
p9.p2cko.na43.temp <- p9.p2cko.na43[row.names(p9.p2cko.na43) %in% ref$GeneSymbol,]
...
P9_P2CKO_NA43 <- as.data.frame(as.matrix(p9.p2cko.na43.temp))


## add gene symbol as a column
P9_P2CKO_NA16$Genes <- row.names(P9_P2CKO_NA16)
P9_P2CKO_NA22$Genes <- row.names(P9_P2CKO_NA22)
P9_P2CKO_NA43$Genes <- row.names(P9_P2CKO_NA43)


## combine individual tables into a giant data frame
dataCombined <- list("P9P2CKONA16" = P9_P2CKO_NA16, "P9P2CKONA22" = P9_P2CKO_NA22, "P9P2CKONA43" = P9_P2CKO_NA43)
...
save(p9.p2cko.data, file = "NA_SEURAT_P2CKO_P9_CB_DF_COUNTS.RData")


##------------------------------------
rm(list = ls())
load("NA_SEURAT_P2CKO_P9_CB_DF_COUNTS.RData")
# p9.p2cko.data

## Initialize the Seurat object with the raw (non-normalized data).
seuObj <- CreateSeuratObject(counts = p9.p2cko.data, project = "NA_P2CKO_P09")
print(dim(seuObj)) 
# 21967 57903
...
seuObj$LibPrep <- gsub("P2CKO_P09_NA16", 1, seuObj$GenoAgeSample)
seuObj$LibPrep <- gsub("P2CKO_P09_NA22", 2, seuObj$LibPrep)
seuObj$LibPrep <- gsub("P2CKO_P09_NA43", 3, seuObj$LibPrep)

seuObj$SeqBatch <- gsub("P2CKO_P09_NA16", 1, seuObj$GenoAgeSample)
seuObj$SeqBatch <- gsub("P2CKO_P09_NA22", 2, seuObj$SeqBatch)
seuObj$SeqBatch <- gsub("P2CKO_P09_NA43", 3, seuObj$SeqBatch)
...
save(seuObj, file = "NA_SEURAT_P2CKO_P9_TEMP.RData")


##------------------------------------
## Add any missing covariates
seuObj$Sex <- gsub("P2CKO_P09_NA16", "F", seuObj$GenoAgeSample)
seuObj$Sex <- gsub("P2CKO_P09_NA22", "M", seuObj$Sex)
seuObj$Sex <- gsub("P2CKO_P09_NA43", "F", seuObj$Sex)
...
##------------------------------------
## Save RData
save(seuObj, file = "NA_SEURAT_P2CKO_P9.RData")

HARMONISED_UMAP <- DimPlot(seuObj, group.by = c("GenoAgeSample", "SeqBatch", "Sex", "RNA_snn_res.0.8"), pt.size = 0.01)
ggsave("NA_SEURAT_P2CKO_P9_UMAP.pdf", plot = HARMONISED_UMAP, width = 16, height = 12, units = "in", dpi = 300)

HARMONISED_UMAP_FACET <- DimPlot(seuObj, group.by = "GenoAgeSample", pt.size = 0.01, split.by = "GenoAgeSample", ncol = 3)
ggsave("NA_SEURAT_P2CKO_P9_UMAP_FACET.pdf", plot = HARMONISED_UMAP_FACET, width = 20, height = 6, units = "in", dpi = 300)
