...
# load("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P09/18_DOUBLETFINDER_P1P2CKO/NA_SEURAT_P1P2CKO_P9_CB_DF.RData")
# # seuObjDF
...
# p9.p1p2cko.na15.f <-  row.names(seuObjDFremoved@meta.data[seuObjDFremoved@meta.data$GenoAgeSample == "P1P2CKO_P09_NA15",])
# p9.p1p2cko.na20.f <-  row.names(seuObjDFremoved@meta.data[seuObjDFremoved@meta.data$GenoAgeSample == "P1P2CKO_P09_NA20",])
# p9.p1p2cko.na24.f <-  row.names(seuObjDFremoved@meta.data[seuObjDFremoved@meta.data$GenoAgeSample == "P1P2CKO_P09_NA24",])

# ## load dataset from CellBender run
# p9.p1p2cko.na15.cb <- Read10X_h5("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P09/01_COUNTS_RAW/NA15_raw_feature_bc_matrix/CellBender_Out_filtered.h5", use.names = TRUE, unique.features = TRUE)
# print(dim(p9.p1p2cko.na15.cb))
# # 53715 19134

# p9.p1p2cko.na20.cb <- Read10X_h5("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P09/01_COUNTS_RAW/NA20_raw_feature_bc_matrix/CellBender_Out_filtered.h5", use.names = TRUE, unique.features = TRUE)
# print(dim(p9.p1p2cko.na20.cb))
# # 53715 12796

# p9.p1p2cko.na24.cb <- Read10X_h5("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P09/01_COUNTS_RAW/NA24_raw_feature_bc_matrix/CellBender_Out_filtered.h5", use.names = TRUE, unique.features = TRUE)
# print(dim(p9.p1p2cko.na24.cb))
# # 53715 19131


# ## adding sample & condition info to column names
# colnames(p9.p1p2cko.na15.cb) <- paste("P1P2CKO_P09_NA15", colnames(p9.p1p2cko.na15.cb), sep = "_")
# colnames(p9.p1p2cko.na20.cb) <- paste("P1P2CKO_P09_NA20", colnames(p9.p1p2cko.na20.cb), sep = "_")
# colnames(p9.p1p2cko.na24.cb) <- paste("P1P2CKO_P09_NA24", colnames(p9.p1p2cko.na24.cb), sep = "_")



# ## fetching counts for common cellbarcodes
# p9.p1p2cko.na15 <- p9.p1p2cko.na15.cb[,colnames(p9.p1p2cko.na15.cb) %in% p9.p1p2cko.na15.f]
# p9.p1p2cko.na20 <- p9.p1p2cko.na20.cb[,colnames(p9.p1p2cko.na20.cb) %in% p9.p1p2cko.na20.f]
# p9.p1p2cko.na24 <- p9.p1p2cko.na24.cb[,colnames(p9.p1p2cko.na24.cb) %in% p9.p1p2cko.na24.f]
...
# save(p9.p1p2cko.data, file = "NA_SEURAT_P1P2CKO_P9_CB_DF_COUNTS.RData")


##------------------------------------
rm(list = ls())
load("NA_SEURAT_P1P2CKO_P9_CB_DF_COUNTS.RData")
# p9.p1p2cko.data

## Initialize the Seurat object with the raw (non-normalized data).
seuObj <- CreateSeuratObject(counts = p9.p1p2cko.data, project = "NA_P1P2CKO_P09")
print(dim(seuObj)) 
# 21967 57903
...
seuObj$LibPrep <- gsub("P1P2CKO_P09_NA15", 1, seuObj$GenoAgeSample)
seuObj$LibPrep <- gsub("P1P2CKO_P09_NA20", 2, seuObj$LibPrep)
seuObj$LibPrep <- gsub("P1P2CKO_P09_NA24", 2, seuObj$LibPrep)

seuObj$SeqBatch <- gsub("P1P2CKO_P09_NA15", 1, seuObj$GenoAgeSample)
seuObj$SeqBatch <- gsub("P1P2CKO_P09_NA20", 2, seuObj$SeqBatch)
seuObj$SeqBatch <- gsub("P1P2CKO_P09_NA24", 2, seuObj$SeqBatch)
...
save(seuObj, file = "NA_SEURAT_P1P2CKO_P9_TEMP.RData")


##------------------------------------
## Add any missing covariates
seuObj$Sex <- gsub("P1P2CKO_P09_NA15", "F", seuObj$GenoAgeSample)
seuObj$Sex <- gsub("P1P2CKO_P09_NA20", "M", seuObj$Sex)
seuObj$Sex <- gsub("P1P2CKO_P09_NA24", "F", seuObj$Sex)


##------------------------------------
## Harmony
seuObj <- RunHarmony(object = seuObj, group.by.vars = c("SeqBatch", "Sex"), reduction = "pca", assay.use = "RNA") #, project.dim = FALSE)
...
##------------------------------------
## Save RData
save(seuObj, file = "NA_SEURAT_P1P2CKO_P9.RData")


HARMONISED_UMAP <- DimPlot(seuObj, group.by = c("GenoAgeSample", "SeqBatch", "Sex", "RNA_snn_res.0.8"), pt.size = 0.01)
ggsave("NA_SEURAT_P1P2CKO_P9_UMAP.pdf", plot = HARMONISED_UMAP, width = 16, height = 12, units = "in", dpi = 300)

HARMONISED_UMAP_FACET <- DimPlot(seuObj, group.by = "GenoAgeSample", pt.size = 0.01, split.by = "GenoAgeSample", ncol = 3)
ggsave("NA_SEURAT_P1P2CKO_P9_UMAP_FACET.pdf", plot = HARMONISED_UMAP_FACET, width = 20, height = 6, units = "in", dpi = 300)
