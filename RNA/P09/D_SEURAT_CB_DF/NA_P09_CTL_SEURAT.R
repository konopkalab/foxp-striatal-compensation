...
# load("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P09/18_DOUBLETFINDER_CTL/NA_SEURAT_CTL_P9_CB_DF.RData")
# # seuObjDF
...
# p9.ctl.na13.f <-  row.names(seuObjDFremoved@meta.data[seuObjDFremoved@meta.data$GenoAgeSample == "CTL_P09_NA13",])
# p9.ctl.na14.f <-  row.names(seuObjDFremoved@meta.data[seuObjDFremoved@meta.data$GenoAgeSample == "CTL_P09_NA14",])
# p9.ctl.na29.f <-  row.names(seuObjDFremoved@meta.data[seuObjDFremoved@meta.data$GenoAgeSample == "CTL_P09_NA29",])

# ## load dataset from CellBender run
# p9.ctl.na13.cb <- Read10X_h5("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P09/01_COUNTS_RAW/NA13_raw_feature_bc_matrix/CellBender_Out_filtered.h5", use.names = TRUE, unique.features = TRUE)
# print(dim(p9.ctl.na13.cb))
# # 53715 13830

# p9.ctl.na14.cb <- Read10X_h5("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P09/01_COUNTS_RAW/NA14_raw_feature_bc_matrix/CellBender_Out_filtered.h5", use.names = TRUE, unique.features = TRUE)
# print(dim(p9.ctl.na14.cb))
# # 53715 19108

# p9.ctl.na29.cb <- Read10X_h5("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P09/01_COUNTS_RAW/NA29_raw_feature_bc_matrix/CellBender_Out_filtered.h5", use.names = TRUE, unique.features = TRUE)
# print(dim(p9.ctl.na29.cb))
# # 53715 18835


# ## adding sample & condition info to column names
# colnames(p9.ctl.na13.cb) <- paste("CTL_P09_NA13", colnames(p9.ctl.na13.cb), sep = "_")
# colnames(p9.ctl.na14.cb) <- paste("CTL_P09_NA14", colnames(p9.ctl.na14.cb), sep = "_")
# colnames(p9.ctl.na29.cb) <- paste("CTL_P09_NA29", colnames(p9.ctl.na29.cb), sep = "_")



# ## fetching counts for common cellbarcodes
# p9.ctl.na13 <- p9.ctl.na13.cb[,colnames(p9.ctl.na13.cb) %in% p9.ctl.na13.f]
# p9.ctl.na14 <- p9.ctl.na14.cb[,colnames(p9.ctl.na14.cb) %in% p9.ctl.na14.f]
# p9.ctl.na29 <- p9.ctl.na29.cb[,colnames(p9.ctl.na29.cb) %in% p9.ctl.na29.f]


# ## fetch genes or rows corresponding to gencode protein coding gene symbols
# p9.ctl.na13.temp <- p9.ctl.na13[row.names(p9.ctl.na13) %in% ref$GeneSymbol,]
# p9.ctl.na14.temp <- p9.ctl.na14[row.names(p9.ctl.na14) %in% ref$GeneSymbol,]
# p9.ctl.na29.temp <- p9.ctl.na29[row.names(p9.ctl.na29) %in% ref$GeneSymbol,]


# ## make a data from from matrix
# P9_CTL_NA13 <- as.data.frame(as.matrix(p9.ctl.na13.temp))
# P9_CTL_NA14 <- as.data.frame(as.matrix(p9.ctl.na14.temp))
# P9_CTL_NA29 <- as.data.frame(as.matrix(p9.ctl.na29.temp))


# ## add gene symbol as a column
# P9_CTL_NA13$Genes <- row.names(P9_CTL_NA13)
# P9_CTL_NA14$Genes <- row.names(P9_CTL_NA14)
# P9_CTL_NA29$Genes <- row.names(P9_CTL_NA29)


# ## combine individual tables into a giant data frame
# dataCombined <- list("P9CTLNA13" = P9_CTL_NA13, "P9CTLNA14" = P9_CTL_NA14, "P9CTLNA29" = P9_CTL_NA29)
...
# save(p9.ctl.data, file = "NA_SEURAT_CTL_P9_CB_DF_COUNTS.RData")


##------------------------------------
rm(list = ls())
load("NA_SEURAT_CTL_P9_CB_DF_COUNTS.RData")
# p9.ctl.data

## Initialize the Seurat object with the raw (non-normalized data).
seuObj <- CreateSeuratObject(counts = p9.ctl.data, project = "NA_CTL_P09")
print(dim(seuObj)) 
# 21967 47890
...
seuObj$LibPrep <- gsub("CTL_P09_NA13", 1, seuObj$GenoAgeSample)
seuObj$LibPrep <- gsub("CTL_P09_NA14", 1, seuObj$LibPrep)
seuObj$LibPrep <- gsub("CTL_P09_NA29", 2, seuObj$LibPrep)

seuObj$SeqBatch <- gsub("CTL_P09_NA13", 1, seuObj$GenoAgeSample)
seuObj$SeqBatch <- gsub("CTL_P09_NA14", 1, seuObj$SeqBatch)
seuObj$SeqBatch <- gsub("CTL_P09_NA29", 2, seuObj$SeqBatch)
...
save(seuObj, file = "NA_SEURAT_CTL_P9_TEMP.RData")

##------------------------------------
## Add any missing covariates
seuObj$Sex <- gsub("CTL_P09_NA13", "M", seuObj$GenoAgeSample)
seuObj$Sex <- gsub("CTL_P09_NA14", "F", seuObj$Sex)
seuObj$Sex <- gsub("CTL_P09_NA29", "M", seuObj$Sex)
...
##------------------------------------
## Save RData
save(seuObj, file = "NA_SEURAT_CTL_P9.RData")



HARMONISED_UMAP <- DimPlot(seuObj, group.by = c("GenoAgeSample", "SeqBatch", "Sex", "RNA_snn_res.0.8"), pt.size = 0.01)
ggsave("NA_SEURAT_CTL_P9_UMAP.pdf", plot = HARMONISED_UMAP, width = 16, height = 12, units = "in", dpi = 300)

HARMONISED_UMAP_FACET <- DimPlot(seuObj, group.by = "GenoAgeSample", pt.size = 0.01, split.by = "GenoAgeSample", ncol = 3)
ggsave("NA_SEURAT_CTL_P9_UMAP_FACET.pdf", plot = HARMONISED_UMAP_FACET, width = 20, height = 6, units = "in", dpi = 300)
