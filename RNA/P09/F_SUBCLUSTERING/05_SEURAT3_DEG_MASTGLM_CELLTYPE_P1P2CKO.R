...
for (myclu in unique(sort(seuObjFilt@active.ident)))
  ...
  cells2 <- WhichCells(str.selected, idents = 'P1P2CKO')
  ...
  summaryCond <- summary(object = mastfix, doLRT = 'GenotypeP1P2CKO')
  summaryDt <- summaryCond$datatable
  p_val <- summaryDt[summaryDt$component == "H", 4]
  genes.return <- summaryDt[summaryDt$component == "H", 1]
  to.return <- data.frame(p_val, row.names = genes.return$primerid)
  ...
  # summaryCond <- summary(object = mastfix_glmer, doLRT = 'GenotypeP2CKO')
  # summaryDt <- summaryCond$datatable
  # p_val <- summaryDt[summaryDt$component == "H", 4]
  # genes.return <- summaryDt[summaryDt$component == "H", 1]
  # to.return <- data.frame(p_val, row.names = genes.return$primerid)
  ...
  ## write dge table
  write.table(mast_allcov_glm, paste("SEURAT_NA_DATA_", cluSelected, "_P1P2CKO_x_CTL_DEG_TABLE_PSEUDOBULK_FULL.txt", sep = ""), row.names = T, col.names = T, quote = F, sep = "\t")
  ...
  save(mast_allcov_glm, avg.exp, degavg, file = paste("SEURAT_NA_DATA_P1P2CKO_X_CTL_", cluSelected, "_PSEUDOBULK.RData", sep = ""))
	}
