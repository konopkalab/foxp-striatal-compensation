...
# p9.p1cko.na18 <- Read10X_h5("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P09/01_COUNTS_RAW/NA18_raw_feature_bc_matrix/CellBender_Out_filtered.h5", use.names = TRUE, unique.features = TRUE)
# print(dim(p9.ctl.na18))

p9.p1cko.na32 <- Read10X_h5("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P09/01_COUNTS_RAW/NA32_raw_feature_bc_matrix/CellBender_Out_filtered.h5", use.names = TRUE, unique.features = TRUE)
print(dim(p9.p1cko.na32))
# 53715 16338

p9.p1cko.na33 <- Read10X_h5("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P09/01_COUNTS_RAW/NA33_raw_feature_bc_matrix/CellBender_Out_filtered.h5", use.names = TRUE, unique.features = TRUE)
print(dim(p9.p1cko.na33))
# 53715 17073

p9.p1cko.na42 <- Read10X_h5("/project/Neuroinformatics_Core/Konopka_lab/akulk1/NEW_SEP2022_NK_NA_EO_CO/03_CELLBENDER_NA/NA42_CellBender_Out_filtered.h5", use.names = TRUE, unique.features = TRUE)
print(dim(p9.p1cko.na42))
# 53715 21394


## adding sample & condition info to column names
# colnames(p9.p1cko.na18) <- paste("P1CKO_P09_NA18", colnames(p9.p1cko.na18), sep = "_")
colnames(p9.p1cko.na32) <- paste("P1CKO_P09_NA32", colnames(p9.p1cko.na32), sep = "_")
colnames(p9.p1cko.na33) <- paste("P1CKO_P09_NA33", colnames(p9.p1cko.na33), sep = "_")
colnames(p9.p1cko.na42) <- paste("P1CKO_P09_NA42", colnames(p9.p1cko.na42), sep = "_")



## fetch genes or rows corresponding to gencode protein coding gene symbols
# p9.p1cko.na18.temp <- p9.p1cko.na18[row.names(p9.p1cko.na18) %in% ref$GeneSymbol,]
...
# P9_P1CKO_NA18 <- as.data.frame(as.matrix(p9.p1cko.na18.temp))
...
# P9_P1CKO_NA18$Genes <- row.names(P9_P1CKO_NA18)
...
# dataCombined <- list("P9P1CKONA18" = P9_P1CKO_NA18, "P9P1CKONA32" = P9_P1CKO_NA32, "P9P1CKONA33" = P9_P1CKO_NA33)
dataCombined <- list("P9P1CKONA32" = P9_P1CKO_NA32, "P9P1CKONA33" = P9_P1CKO_NA33, "P9_P1CKO_NA42" = P9_P1CKO_NA42)
...
save(p9.p1cko.data, file = "NA_COUNTS_P1CKO_P9_CB.RData")


rm(list = ls())
load("NA_COUNTS_P1CKO_P9_CB.RData")

##------------------------------------
## Initialize the Seurat object with the raw (non-normalized data).
seuObj <- CreateSeuratObject(counts = p9.p1cko.data, project = "NA_P1CKO_P9")
print(dim(seuObj)) 
## 21967 92343
...
##------------------------------------
## Save RData
save(seuObj, file = "NA_SEURAT_P1CKO_P9_CB.RData")
...
# ## SEURAT PLOTS
# ##------------------------------------
# load("NA_SEURAT_P1CKO_P9_CB.RData")
# # seuObj
...
# ggsave(filename = "NK_SEURAT_P1CKO_P1_CLUSTREE.pdf", plot = seutree, width = 24, height = 24, units = "in", dpi = 150)
...
#     pdf(paste0("NK_SEURAT_P1CKO_P1_UMAP_RES_", Resolution, ".pdf"), width = 7, height = 6)
#     g <- DimPlot(object = seuObj, label = TRUE, reduction = "umap", group.by = Resolution)
#     print(g)
#     dev.off()

#     pdf(paste0("NK_SEURAT_P1CKO_P1_VIOLIN_nUMI_RES_", Resolution, ".pdf"), width = 7, height = 3)
...
save(seuObjDF, selpcs, selpk, nExp_poi.adj, file = "NA_SEURAT_P1CKO_P9_CB_DF_temp.RData")


seuObjDF <- doubletFinder_v3(seuObjDF, PCs = 1:selpcs, pN = 0.25, pK = selpk, nExp = nExp_poi.adj, reuse.pANN = "pANN_0.25_0.005_4110", sct = FALSE)

table(seuObjDF$GenoAgeSample, seuObjDF$DF.classifications_0.25_0.005_4110)
#                  Doublet Singlet
#   P1CKO_P09_NA32      66   16272
#   P1CKO_P09_NA33    1285   15788
#   P1CKO_P09_NA42    2759   18635

table(seuObjDF$GenoAgeSample, seuObjDF$DF.classifications_0.25_0.005_3810)
#                  Doublet Singlet
#   P1CKO_P09_NA32      56   16282
#   P1CKO_P09_NA33    1189   15884
#   P1CKO_P09_NA42    2565   18829

seuMeta <- as.data.frame(seuObjDF@meta.data)
seuMetaFiltTemp <- seuMeta[seuMeta$DF.classifications_0.25_0.005_4110 == "Singlet",]
seuMetaFilt <- seuMetaFiltTemp[seuMetaFiltTemp$DF.classifications_0.25_0.005_3810 == "Singlet",]
...
##------------------------------------
## Save RData
save(seuObjDF, file = "NA_SEURAT_P1CKO_P9_CB_DF.RData")
save(seuMeta, seuMetaFilt, file = "NA_SEURAT_P1CKO_P9_CB_DF_FILT_META.RData")
...
# ## Run DoubletFinder with varying classification stringencies ----------------------------------------------------------------
# seuObjDF <- doubletFinder_v3(seuObj, PCs = 1:selpcs, pN = 0.25, pK = selpk, nExp = nExp_poi, reuse.pANN = FALSE, sct = FALSE)
# seuObjDF <- doubletFinder_v3(seuObjDF, PCs = 1:selpcs, pN = 0.25, pK = selpk, nExp = nExp_poi.adj, reuse.pANN = "DF.classifications_0.25_0.005_2494", sct = FALSE)

# table(seuObjDF$GenoAgeSample, seuObjDF$DF.classifications_0.25_0.005_2494)
# #                 Doublet Singlet
# #   P1CKO_P9_NA32     713   15625
# #   P1CKO_P9_NA33    1781   15130

# table(seuObjDF$GenoAgeSample, seuObjDF$DF.classifications_0.25_0.005_2258)
# #                 Doublet Singlet
# #   P1CKO_P9_NA32    2258   14080
# #   P1CKO_P9_NA33       0   16911
...
# save(seuObj, file = "NK_SEURAT_P1CKO_P1_UPDATED.RData")
...
# load("NK_SEURAT_P1CKO_P1_UPDATED.RData")
# # seuObj
...
# save(seuObj, file = "NK_SEURAT_P1CKO_P1_UPDATED_WPRE_NEW.RData")
...
# # save(seuObj, file = "NK_SEURAT_P1CKO_P1_UPDATED_WPRE.RData")
