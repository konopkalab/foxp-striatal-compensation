...
# p9.p2cko.na16 <- Read10X_h5("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P09/01_COUNTS_RAW/NA16_raw_feature_bc_matrix/CellBender_Out_filtered.h5", use.names = TRUE, unique.features = TRUE)
# print(dim(p9.p2cko.na16))
# # 53715 11380

# p9.p2cko.na22 <- Read10X_h5("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P09/01_COUNTS_RAW/NA22_raw_feature_bc_matrix/CellBender_Out_filtered.h5", use.names = TRUE, unique.features = TRUE)
# print(dim(p9.p2cko.na22))
# # 53715 13680

# p9.p1cko.na43 <- Read10X_h5("/project/Neuroinformatics_Core/Konopka_lab/akulk1/NEW_SEP2022_NK_NA_EO_CO/03_CELLBENDER_NA/NA43_CellBender_Out_filtered.h5", use.names = TRUE, unique.features = TRUE)
# print(dim(p9.p1cko.na43))
# # 53715 37538


# ## adding sample & condition info to column names
# colnames(p9.p2cko.na16) <- paste("P2CKO_P09_NA16", colnames(p9.p2cko.na16), sep = "_")
# colnames(p9.p2cko.na22) <- paste("P2CKO_P09_NA22", colnames(p9.p2cko.na22), sep = "_")
# colnames(p9.p1cko.na43) <- paste("P2CKO_P09_NA43", colnames(p9.p1cko.na43), sep = "_")


# ## fetch genes or rows corresponding to gencode protein coding gene symbols
# p9.p2cko.na16.temp <- p9.p2cko.na16[row.names(p9.p2cko.na16) %in% ref$GeneSymbol,]
# p9.p2cko.na22.temp <- p9.p2cko.na22[row.names(p9.p2cko.na22) %in% ref$GeneSymbol,]
# p9.p1cko.na43.temp <- p9.p1cko.na43[row.names(p9.p1cko.na43) %in% ref$GeneSymbol,]


# ## make a data from from matrix
# P9_P2CKO_NA16 <- as.data.frame(as.matrix(p9.p2cko.na16.temp))
# P9_P2CKO_NA22 <- as.data.frame(as.matrix(p9.p2cko.na22.temp))
# P9_P1CKO_NA43 <- as.data.frame(as.matrix(p9.p1cko.na43.temp))


# ## add gene symbol as a column
# P9_P2CKO_NA16$Genes <- row.names(P9_P2CKO_NA16)
# P9_P2CKO_NA22$Genes <- row.names(P9_P2CKO_NA22)
# P9_P1CKO_NA43$Genes <- row.names(P9_P1CKO_NA43)


# ## combine individual tables into a giant data frame
# dataCombined <- list("P9P2CKONA16" = P9_P2CKO_NA16, "P9P2CKONA22" = P9_P2CKO_NA22, "P9P2CKONA43" = P9_P1CKO_NA43)
...
# p9.p2cko.data <- na.data

# save(p9.p2cko.data, file = "NA_COUNTS_P2CKO_P9_CB.RData")




rm(list = ls())
load("NA_COUNTS_P2CKO_P9_CB.RData")

##------------------------------------
## Initialize the Seurat object with the raw (non-normalized data).
seuObj <- CreateSeuratObject(counts = p9.p2cko.data, project = "NA_P2CKO_P9")
print(dim(seuObj)) 
...
##------------------------------------
## Save RData
save(seuObj, file = "NA_SEURAT_P2CKO_P9_CB.RData")
...
# ## SEURAT PLOTS
# ##------------------------------------
# load("NA_SEURAT_P2CKO_P9_CB.RData")
# # seuObj
...
# ggsave(filename = "NK_SEURAT_P2CKO_P18_CLUSTREE.pdf", plot = seutree, width = 24, height = 24, units = "in", dpi = 150)
...
#     pdf(paste0("NK_SEURAT_P2CKO_P1_UMAP_RES_", Resolution, ".pdf"), width = 7, height = 6)
#     g <- DimPlot(object = seuObj, label = TRUE, reduction = "umap", group.by = Resolution)
#     print(g)
#     dev.off()

#     pdf(paste0("NK_SEURAT_P2CKO_P1_VIOLIN_nUMI_RES_", Resolution, ".pdf"), width = 7, height = 3)
...
save(seuObjDF, selpcs, selpk, nExp_poi.adj, file = "NA_SEURAT_P2CKO_P9_CB_DF_temp.RData")


seuObjDF <- doubletFinder_v3(seuObjDF, PCs = 1:selpcs, pN = 0.25, pK = selpk, nExp = nExp_poi.adj, reuse.pANN = "pANN_0.25_0.005_4695", sct = FALSE)

table(seuObjDF$GenoAgeSample, seuObjDF$DF.classifications_0.25_0.005_4695)
#                  Doublet Singlet
#   P2CKO_P09_NA16    1311   10069
#   P2CKO_P09_NA22    1320   12360
#   P2CKO_P09_NA43    2064   35474

table(seuObjDF$GenoAgeSample, seuObjDF$DF.classifications_0.25_0.005_4418)
#                  Doublet Singlet
#   P2CKO_P09_NA16    1218   10162
#   P2CKO_P09_NA22    1260   12420
#   P2CKO_P09_NA43    1940   35598


seuMeta <- as.data.frame(seuObjDF@meta.data)
seuMetaFiltTemp <- seuMeta[seuMeta$DF.classifications_0.25_0.005_4695 == "Singlet",]
seuMetaFilt <- seuMetaFiltTemp[seuMetaFiltTemp$DF.classifications_0.25_0.005_4418 == "Singlet",]

## Before Doublets Removal
table(seuMeta$GenoAgeSample)
# P2CKO_P09_NA16 P2CKO_P09_NA22 P2CKO_P09_NA43 
#          11380          13680          37538
...
##------------------------------------
## Save RData
save(seuObjDF, file = "NA_SEURAT_P2CKO_P9_CB_DF.RData")
save(seuMeta, seuMetaFilt, file = "NA_SEURAT_P2CKO_P9_CB_DF_FILT_META.RData")
...
# save(seuObj, file = "NK_SEURAT_P2CKO_P1_UPDATED.RData")
...
# load("NK_SEURAT_P2CKO_P1_UPDATED.RData")
# # seuObj
...
# save(seuObj, file = "NK_SEURAT_P2CKO_P1_UPDATED_WPRE_NEW.RData")
...
# # save(seuObj, file = "NK_SEURAT_P2CKO_P1_UPDATED_WPRE.RData")
