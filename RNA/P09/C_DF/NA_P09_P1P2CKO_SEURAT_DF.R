...
# p9.p1p2cko.na15 <- Read10X_h5("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P09/01_COUNTS_RAW/NA15_raw_feature_bc_matrix/CellBender_Out_filtered.h5", use.names = TRUE, unique.features = TRUE)
# print(dim(p9.p1p2cko.na15))
# # 53715 19134

# p9.p1p2cko.na20 <- Read10X_h5("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P09/01_COUNTS_RAW/NA20_raw_feature_bc_matrix/CellBender_Out_filtered.h5", use.names = TRUE, unique.features = TRUE)
# print(dim(p9.p1p2cko.na20))
# # 53715 12796

# p9.p1p2cko.na24 <- Read10X_h5("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P09/01_COUNTS_RAW/NA24_raw_feature_bc_matrix/CellBender_Out_filtered.h5", use.names = TRUE, unique.features = TRUE)
# print(dim(p9.p1p2cko.na24))
# # 53715 19131


# ## adding sample & condition info to column names
# colnames(p9.p1p2cko.na15) <- paste("P1P2CKO_P09_NA15", colnames(p9.p1p2cko.na15), sep = "_")
# colnames(p9.p1p2cko.na20) <- paste("P1P2CKO_P09_NA20", colnames(p9.p1p2cko.na20), sep = "_")
# colnames(p9.p1p2cko.na24) <- paste("P1P2CKO_P09_NA24", colnames(p9.p1p2cko.na24), sep = "_")


# ## fetch genes or rows corresponding to gencode protein coding gene symbols
# p9.p1p2cko.na15.temp <- p9.p1p2cko.na15[row.names(p9.p1p2cko.na15) %in% ref$GeneSymbol,]
# p9.p1p2cko.na20.temp <- p9.p1p2cko.na20[row.names(p9.p1p2cko.na20) %in% ref$GeneSymbol,]
# p9.p1p2cko.na24.temp <- p9.p1p2cko.na24[row.names(p9.p1p2cko.na24) %in% ref$GeneSymbol,]


# ## make a data from from matrix
# P9_P1P2CKO_NA15 <- as.data.frame(as.matrix(p9.p1p2cko.na15.temp))
# P9_P1P2CKO_NA20 <- as.data.frame(as.matrix(p9.p1p2cko.na20.temp))
# P9_P1P2CKO_NA24 <- as.data.frame(as.matrix(p9.p1p2cko.na24.temp))


# ## add gene symbol as a column
# P9_P1P2CKO_NA15$Genes <- row.names(P9_P1P2CKO_NA15)
# P9_P1P2CKO_NA20$Genes <- row.names(P9_P1P2CKO_NA20)
# P9_P1P2CKO_NA24$Genes <- row.names(P9_P1P2CKO_NA24)


# ## combine individual tables into a giant data frame
# dataCombined <- list("P9P1P2CKONA15" = P9_P1P2CKO_NA15, "P9P1P2CKONA20" = P9_P1P2CKO_NA20, "P9P1P2CKONA24" = P9_P1P2CKO_NA24)
...
# p9.p1p2cko.data <- na.data

# save(p9.p1p2cko.data, file = "NA_COUNTS_P1P2CKO_P9_CB.RData")


rm(list = ls())
load("NA_COUNTS_P1P2CKO_P9_CB.RData")

##------------------------------------
## Initialize the Seurat object with the raw (non-normalized data).
seuObj <- CreateSeuratObject(counts = p9.p1p2cko.data, project = "NA_P1P2CKO_P9")
print(dim(seuObj)) 
## 21967 51061
...
##------------------------------------
## Save RData
save(seuObj, file = "NA_SEURAT_P1P2CKO_P9_CB.RData")
...
load("NA_SEURAT_P1P2CKO_P9_CB.RData")
# seuObj
...
save(seuObjDF, selpcs, selpk, nExp_poi.adj, file = "NA_SEURAT_P1P2CKO_P9_CB_DF_temp.RData")


seuObjDF <- doubletFinder_v3(seuObjDF, PCs = 1:selpcs, pN = 0.25, pK = selpk, nExp = nExp_poi.adj, reuse.pANN = "pANN_0.25_0.005_3830", sct = FALSE)

table(seuObjDF$GenoAgeSample, seuObjDF$DF.classifications_0.25_0.005_3830)
#                    Doublet Singlet
#   P1P2CKO_P09_NA15    1627   17507
#   P1P2CKO_P09_NA20    1206   11590
#   P1P2CKO_P09_NA24     997   18134

table(seuObjDF$GenoAgeSample, seuObjDF$DF.classifications_0.25_0.005_3629)
#                    Doublet Singlet
#   P1P2CKO_P09_NA15    1547   17587
#   P1P2CKO_P09_NA20    1147   11649
#   P1P2CKO_P09_NA24     935   18196

seuMeta <- as.data.frame(seuObjDF@meta.data)
seuMetaFiltTemp <- seuMeta[seuMeta$DF.classifications_0.25_0.005_3830 == "Singlet",]
seuMetaFilt <- seuMetaFiltTemp[seuMetaFiltTemp$DF.classifications_0.25_0.005_3629 == "Singlet",]

## Before Doublets Removal
table(seuMeta$GenoAgeSample)
# P1P2CKO_P09_NA15 P1P2CKO_P09_NA20 P1P2CKO_P09_NA24 
#            19134            12796            19131
...
##------------------------------------
## Save RData
save(seuObjDF, file = "NA_SEURAT_P1P2CKO_P9_CB_DF.RData")
save(seuMeta, seuMetaFilt, file = "NA_SEURAT_P1P2CKO_P9_CB_DF_FILT_META.RData")
...
# ggsave(filename = "NK_SEURAT_P1P2CKO_P18_CLUSTREE.pdf", plot = seutree, width = 24, height = 24, units = "in", dpi = 150)
...
#     pdf(paste0("NK_SEURAT_P1P2CKO_P1_UMAP_RES_", Resolution, ".pdf"), width = 7, height = 6)
#     g <- DimPlot(object = seuObj, label = TRUE, reduction = "umap", group.by = Resolution)
#     print(g)
#     dev.off()

#     pdf(paste0("NK_SEURAT_P1P2CKO_P1_VIOLIN_nUMI_RES_", Resolution, ".pdf"), width = 7, height = 3)
...
# ## Run DoubletFinder with varying classification stringencies ----------------------------------------------------------------
# seuObjDF <- doubletFinder_v3(seuObj, PCs = 1:selpcs, pN = 0.25, pK = selpk, nExp = nExp_poi, reuse.pANN = FALSE, sct = FALSE)
# seuObjDF <- doubletFinder_v3(seuObjDF, PCs = 1:selpcs, pN = 0.25, pK = selpk, nExp = nExp_poi.adj, reuse.pANN = "DF.classifications_0.25_0.005_3744", sct = FALSE)

# table(seuObjDF$GenoAgeSample, seuObjDF$DF.classifications_0.25_0.005_3744)
# #                   Doublet Singlet
# #   P1P2CKO_P9_NA15    1503   17029
# #   P1P2CKO_P9_NA20    1226   11560
# #   P1P2CKO_P9_NA24    1015   17583

# table(seuObjDF$GenoAgeSample, seuObjDF$DF.classifications_0.25_0.005_3494)
# #                   Doublet Singlet
# #   P1P2CKO_P9_NA15    3494   15038
# #   P1P2CKO_P9_NA20       0   12786
# #   P1P2CKO_P9_NA24       0   18598
...
# ## Plots
# load("NA_SEURAT_P1P2CKO_P9.RData")
# # seuObj
...
# rm(list = ls())
# load("NK_SEURAT_P1P2CKO_P1.RData")
# # seuObj
...
# save(seuObj, file = "NK_SEURAT_P1P2CKO_P1_UPDATED.RData")
...
# load("NK_SEURAT_P1P2CKO_P1_UPDATED.RData")
# # seuObj
...
# save(seuObj, file = "NK_SEURAT_P1P2CKO_P1_UPDATED_WPRE_NEW.RData")
...
# # save(seuObj, file = "NK_SEURAT_P1P2CKO_P1_UPDATED_WPRE.RData")
