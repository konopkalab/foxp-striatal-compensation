...
##-------------------------------------------------------
## FETCH THE LIST OF GENES EXPRESSED ACROSS ALL CELLTYPES
rm(list = ls())
library(dplyr)
library(Seurat)
library(patchwork)
...
gene.expression <- apply(seuObjFilt@assays$RNA@data, 1, mean)
...
write.table(exp_genes_sorted, "NA_P09_ALL_CELLTYPES_EXPRESSED_GENES.txt", row.names = FALSE, col.names = FALSE, quote = FALSE, sep = "\t")


gene.expression.sorted2 <- as.data.frame(gene.expression.sorted)
write.table(gene.expression.sorted2, "NA_P09_ALL_CELLTYPES_ALL_GENES.txt", row.names = TRUE, col.names = FALSE, quote = FALSE, sep = "\t")
