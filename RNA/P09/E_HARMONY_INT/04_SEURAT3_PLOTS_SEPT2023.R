...
table(seuObjSel@meta.data$CellType)
#          A_PROG B_NEURONAL_PROG          C_dSPN          D_iSPN          E_eSPN 
#            5783           10448           51235           49759            7469 
#         F_ASTRO          G_OLIG         H_MICRO     I_ENDO_EPEN         J_INTER 
#           24143           18305            6871            5798           10271 
#      K_CORTICAL 
#            2300 

table(seuObjSel@meta.data$Genotype)
#     CTL   P1CKO P1P2CKO   P2CKO 
#   46307   48801   45745   51529 

table(seuObjSel@meta.data$GenoAgeSample)
#     CTL_P09_NA13     CTL_P09_NA14     CTL_P09_NA29   P1CKO_P09_NA32 
#            11930            16965            17412            15884 
#   P1CKO_P09_NA33   P1CKO_P09_NA42 P1P2CKO_P09_NA15 P1P2CKO_P09_NA20 
#            15009            17908            17008            11349 
# P1P2CKO_P09_NA24   P2CKO_P09_NA16   P2CKO_P09_NA22   P2CKO_P09_NA43 
#            17388             6984            11466            33079


seuObjFiltCTL <- subset(seuObjSel, subset = Genotype == c("CTL"))
...
seuObjFiltCTL_astro <- subset(seuObjFiltCTL, ident = c("F_ASTRO"))
seuObjFiltCTL_oligo <- subset(seuObjFiltCTL, ident = c("G_OLIG"))
seuObjFiltCTL_micro <- subset(seuObjFiltCTL, ident = c("H_MICRO"))
seuObjFiltCTL_endoepen <- subset(seuObjFiltCTL, ident = c("I_ENDO_EPEN"))
seuObjFiltCTL_inter <- subset(seuObjFiltCTL, ident = c("J_INTER"))
seuObjFiltCTL_cortical <- subset(seuObjFiltCTL, ident = c("K_CORTICAL"))
...
gene.expression.astro <- as.data.frame(apply(seuObjFiltCTL_astro@assays$RNA@counts, 1, sum))
gene.expression.oligo <- as.data.frame(apply(seuObjFiltCTL_oligo@assays$RNA@counts, 1, sum))
gene.expression.micro <- as.data.frame(apply(seuObjFiltCTL_micro@assays$RNA@counts, 1, sum))
gene.expression.endoepen <- as.data.frame(apply(seuObjFiltCTL_endoepen@assays$RNA@counts, 1, sum))
gene.expression.inter <- as.data.frame(apply(seuObjFiltCTL_inter@assays$RNA@counts, 1, sum))
gene.expression.cortical <- as.data.frame(apply(seuObjFiltCTL_cortical@assays$RNA@counts, 1, sum))
...
colnames(gene.expression.astro) <- c("Astro")
colnames(gene.expression.oligo) <- c("Oligo")
colnames(gene.expression.micro) <- c("Micro")
colnames(gene.expression.endoepen) <- c("EndoEpen")
colnames(gene.expression.inter) <- c("Inter")
colnames(gene.expression.cortical) <- c("Cortical")
...
gene.expression.astro$Genes <- row.names(gene.expression.astro)
gene.expression.oligo$Genes <- row.names(gene.expression.oligo)
gene.expression.micro$Genes <- row.names(gene.expression.micro)
gene.expression.endoepen$Genes <- row.names(gene.expression.endoepen)
gene.expression.inter$Genes <- row.names(gene.expression.inter)
gene.expression.cortical$Genes <- row.names(gene.expression.cortical)
...
gene.expression.astro.sorted <- gene.expression.astro[order(gene.expression.astro$Genes),]
gene.expression.oligo.sorted <- gene.expression.oligo[order(gene.expression.oligo$Genes),]
gene.expression.micro.sorted <- gene.expression.micro[order(gene.expression.micro$Genes),]
gene.expression.endoepen.sorted <- gene.expression.endoepen[order(gene.expression.endoepen$Genes),]
gene.expression.inter.sorted <- gene.expression.inter[order(gene.expression.inter$Genes),]
gene.expression.cortical.sorted <- gene.expression.cortical[order(gene.expression.cortical$Genes),]
...
dataCombined <- list("prog" = gene.expression.prog.sorted,
                     ...
                     "astro" = gene.expression.astro.sorted,
                     "oligo" = gene.expression.oligo.sorted,
                     "micro" = gene.expression.micro.sorted,
                     "endoepen" = gene.expression.endoepen.sorted,
                     "inter" = gene.expression.inter.sorted,
                     "cortical" = gene.expression.cortical.sorted)

combinedData <- Reduce(function(x, y) { merge(x, y, all = TRUE, by = "Genes") } , dataCombined)
write.table(combinedData, "SEPT2023_NA_P09_CTL_ALL_CELLTYPES_ALL_GENES_COUNTS.txt", row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")
...
# gene.expression.astro <- apply(seuObjFiltCTL_astro@assays$RNA@data, 1, mean)
# gene.expression.oligo <- apply(seuObjFiltCTL_oligo@assays$RNA@data, 1, mean)
# gene.expression.micro <- apply(seuObjFiltCTL_micro@assays$RNA@data, 1, mean)
# gene.expression.endoepen <- apply(seuObjFiltCTL_endoepen@assays$RNA@data, 1, mean)
# gene.expression.inter <- apply(seuObjFiltCTL_inter@assays$RNA@data, 1, mean)
# gene.expression.cortical <- apply(seuObjFiltCTL_cortical@assays$RNA@data, 1, mean)
...
# gene.expression.astro.sorted <- sort(gene.expression.astro, decreasing = TRUE)
# gene.expression.oligo.sorted <- sort(gene.expression.oligo, decreasing = TRUE)
# gene.expression.micro.sorted <- sort(gene.expression.micro, decreasing = TRUE)
# gene.expression.endoepen.sorted <- sort(gene.expression.endoepen, decreasing = TRUE)
# gene.expression.inter.sorted <- sort(gene.expression.inter, decreasing = TRUE)
# gene.expression.cortical.sorted <- sort(gene.expression.cortical, decreasing = TRUE)
...
# expressed.genes.astro <- gene.expression.astro.sorted[gene.expression.astro.sorted > 0]
# expressed.genes.oligo <- gene.expression.oligo.sorted[gene.expression.oligo.sorted > 0]
# expressed.genes.micro <- gene.expression.micro.sorted[gene.expression.micro.sorted > 0]
# expressed.genes.endoepen <- gene.expression.endoepen.sorted[gene.expression.endoepen.sorted > 0]
# expressed.genes.inter <- gene.expression.inter.sorted[gene.expression.inter.sorted > 0]
# expressed.genes.cortical <- gene.expression.cortical.sorted[gene.expression.cortical.sorted > 0]
...
# exp_genes_sorted_astro <- names(expressed.genes.astro)
# exp_genes_sorted_oligo <- names(expressed.genes.oligo)
# exp_genes_sorted_micro <- names(expressed.genes.micro)
# exp_genes_sorted_endoepen <- names(expressed.genes.endoepen)
# exp_genes_sorted_inter <- names(expressed.genes.inter)
# exp_genes_sorted_cortical <- names(expressed.genes.cortical)
...
# gene.expression.astro.sorted2 <- as.data.frame(gene.expression.astro.sorted)
# gene.expression.oligo.sorted2 <- as.data.frame(gene.expression.oligo.sorted)
# gene.expression.micro.sorted2 <- as.data.frame(gene.expression.micro.sorted)
# gene.expression.endoepen.sorted2 <- as.data.frame(gene.expression.endoepen.sorted)
# gene.expression.inter.sorted2 <- as.data.frame(gene.expression.inter.sorted)
# gene.expression.cortical.sorted2 <- as.data.frame(gene.expression.cortical.sorted)

# # write.table(exp_genes_sorted_astro, "NA_P09_CTL_ASTRO_EXPRESSED_GENES.txt", row.names = FALSE, col.names = FALSE, quote = FALSE, sep = "\t")
# # write.table(exp_genes_sorted_oligo, "NA_P09_CTL_OLIGO_EXPRESSED_GENES.txt", row.names = FALSE, col.names = FALSE, quote = FALSE, sep = "\t")

# # write.table(gene.expression.astro.sorted2, "NA_P09_CTL_ASTRO_ALL_GENES.txt", row.names = TRUE, col.names = FALSE, quote = FALSE, sep = "\t")
# # write.table(gene.expression.oligo.sorted2, "NA_P09_CTL_OLIGO_ALL_GENES.txt", row.names = TRUE, col.names = FALSE, quote = FALSE, sep = "\t")

# colnames(gene.expression.prog.sorted2) <- c("Prog")
# colnames(gene.expression.neuroprog.sorted2) <- c("NeuroProg")
# colnames(gene.expression.dspn.sorted2) <- c("dSPN")
# colnames(gene.expression.ispn.sorted2) <- c("iSPN")
# colnames(gene.expression.espn.sorted2) <- c("eSPN")
# colnames(gene.expression.astro.sorted2) <- c("Astro")
# colnames(gene.expression.oligo.sorted2) <- c("Oligo")
# colnames(gene.expression.micro.sorted2) <- c("Micro")
# colnames(gene.expression.endoepen.sorted2) <- c("EndoEpen")
# colnames(gene.expression.inter.sorted2) <- c("Inter")
# colnames(gene.expression.cortical.sorted2) <- c("Cortical")


# gene.expression.prog.sorted2$Genes <- row.names(gene.expression.prog.sorted2)
# gene.expression.neuroprog.sorted2$Genes <- row.names(gene.expression.neuroprog.sorted2)
# gene.expression.dspn.sorted2$Genes <- row.names(gene.expression.dspn.sorted2)
# gene.expression.ispn.sorted2$Genes <- row.names(gene.expression.ispn.sorted2)
# gene.expression.espn.sorted2$Genes <- row.names(gene.expression.espn.sorted2)
# gene.expression.astro.sorted2$Genes <- row.names(gene.expression.astro.sorted2)
# gene.expression.oligo.sorted2$Genes <- row.names(gene.expression.oligo.sorted2)
# gene.expression.micro.sorted2$Genes <- row.names(gene.expression.micro.sorted2)
# gene.expression.endoepen.sorted2$Genes <- row.names(gene.expression.endoepen.sorted2)
# gene.expression.inter.sorted2$Genes <- row.names(gene.expression.inter.sorted2)
# gene.expression.cortical.sorted2$Genes <- row.names(gene.expression.cortical.sorted2)


# gene.expression.prog.sorted3 <- gene.expression.prog.sorted2[order(gene.expression.prog.sorted2$Genes),]
# gene.expression.neuroprog.sorted3 <- gene.expression.neuroprog.sorted2[order(gene.expression.neuroprog.sorted2$Genes),]
# gene.expression.dspn.sorted3 <- gene.expression.dspn.sorted2[order(gene.expression.dspn.sorted2$Genes),]
# gene.expression.ispn.sorted3 <- gene.expression.ispn.sorted2[order(gene.expression.ispn.sorted2$Genes),]
# gene.expression.espn.sorted3 <- gene.expression.espn.sorted2[order(gene.expression.espn.sorted2$Genes),]
# gene.expression.astro.sorted3 <- gene.expression.astro.sorted2[order(gene.expression.astro.sorted2$Genes),]
# gene.expression.oligo.sorted3 <- gene.expression.oligo.sorted2[order(gene.expression.oligo.sorted2$Genes),]
# gene.expression.micro.sorted3 <- gene.expression.micro.sorted2[order(gene.expression.micro.sorted2$Genes),]
# gene.expression.endoepen.sorted3 <- gene.expression.endoepen.sorted2[order(gene.expression.endoepen.sorted2$Genes),]
# gene.expression.inter.sorted3 <- gene.expression.inter.sorted2[order(gene.expression.inter.sorted2$Genes),]
# gene.expression.cortical.sorted3 <- gene.expression.cortical.sorted2[order(gene.expression.cortical.sorted2$Genes),]


# ## combine individual tables into a giant data frame
# dataCombined <- list("prog" = gene.expression.prog.sorted3,
#                      "neuroprog" = gene.expression.neuroprog.sorted3,
#                      "dspn" = gene.expression.dspn.sorted3,
#                      "ispn" = gene.expression.ispn.sorted3,
#                      "espn" = gene.expression.espn.sorted3,
#                      "astro" = gene.expression.astro.sorted3,
#                      "oligo" = gene.expression.oligo.sorted3,
#                      "micro" = gene.expression.micro.sorted3,
#                      "endoepen" = gene.expression.endoepen.sorted3,
#                      "inter" = gene.expression.inter.sorted3,
#                      "cortical" = gene.expression.cortical.sorted3)

# combinedData <- Reduce(function(x, y) { merge(x, y, all = TRUE, by = "Genes") } , dataCombined)
# write.table(combinedData, "SEPT2023_NA_P09_CTL_ALL_CELLTYPES_ALL_GENES.txt", row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")
