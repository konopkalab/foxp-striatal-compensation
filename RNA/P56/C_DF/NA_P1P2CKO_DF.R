...
# p1p2cko.na02 <- Read10X_h5("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P56/02_CELLBENDER/P1P2CKO_NA02/CellBender_Out_filtered.h5", use.names = TRUE, unique.features = TRUE)
# print(dim(p1p2cko.na02)) # 53715 8153

# p1p2cko.na04a <- Read10X_h5("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P56/02_CELLBENDER/P1P2CKO_NA04_01/CellBender_Out_filtered.h5", use.names = TRUE, unique.features = TRUE)
# print(dim(p1p2cko.na04a)) # 53715 27437

# p1p2cko.na04b <- Read10X_h5("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P56/02_CELLBENDER/P1P2CKO_NA04_02/CellBender_Out_filtered.h5", use.names = TRUE, unique.features = TRUE)
# print(dim(p1p2cko.na04b)) # 53715 28536

# p1p2cko.na09 <- Read10X_h5("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P56/02_CELLBENDER/P1P2CKO_NA09/CellBender_Out_filtered.h5", use.names = TRUE, unique.features = TRUE)
# print(dim(p1p2cko.na09)) # 53715 13623


# ## adding sample & condition info to column names
# colnames(p1p2cko.na02) <- paste("P56_P1P2CKO_NA02", colnames(p1p2cko.na02), sep = "_")
# colnames(p1p2cko.na04a) <- paste("P56_P1P2CKO_NA04A", colnames(p1p2cko.na04a), sep = "_")
# colnames(p1p2cko.na04b) <- paste("P56_P1P2CKO_NA04B", colnames(p1p2cko.na04b), sep = "_")
# colnames(p1p2cko.na09) <- paste("P56_P1P2CKO_NA09", colnames(p1p2cko.na09), sep = "_")


# ## fetch genes or rows corresponding to gencode protein coding gene symbols
# p1p2cko.na02.temp <- p1p2cko.na02[row.names(p1p2cko.na02) %in% ref$GeneSymbol,]
# p1p2cko.na04a.temp <- p1p2cko.na04a[row.names(p1p2cko.na04a) %in% ref$GeneSymbol,]
# p1p2cko.na04b.temp <- p1p2cko.na04b[row.names(p1p2cko.na04b) %in% ref$GeneSymbol,]
# p1p2cko.na09.temp <- p1p2cko.na09[row.names(p1p2cko.na09) %in% ref$GeneSymbol,]


# ## make a data from from matrix
# P56_P1P2CKO_NA02 <- as.data.frame(as.matrix(p1p2cko.na02.temp))
# P56_P1P2CKO_NA04A <- as.data.frame(as.matrix(p1p2cko.na04a.temp))
# P56_P1P2CKO_NA04B <- as.data.frame(as.matrix(p1p2cko.na04b.temp))
# P56_P1P2CKO_NA09 <- as.data.frame(as.matrix(p1p2cko.na09.temp))


# ## add gene symbol as a column
# P56_P1P2CKO_NA02$Genes <- row.names(P56_P1P2CKO_NA02)
# P56_P1P2CKO_NA04A$Genes <- row.names(P56_P1P2CKO_NA04A)
# P56_P1P2CKO_NA04B$Genes <- row.names(P56_P1P2CKO_NA04B)
# P56_P1P2CKO_NA09$Genes <- row.names(P56_P1P2CKO_NA09)


# ## combine individual tables into a giant data frame
# dataCombined <- list("P56_P1P2CKO_NA02" = P56_P1P2CKO_NA02, "P56_P1P2CKO_NA04A" = P56_P1P2CKO_NA04A, "P56_P1P2CKO_NA04B" = P56_P1P2CKO_NA04B, "P56_P1P2CKO_NA09" = P56_P1P2CKO_NA09)
...
# print(dim(na.data)) ## 21967 77749

# p56.p1p2cko.data <- na.data

# save(p56.p1p2cko.data, file = "NA_P56_P1P2CKO_CB_COUNTS.RData")



rm(list = ls())
load("NA_P56_P1P2CKO_CB_COUNTS.RData")

##------------------------------------
## Initialize the Seurat object with the raw (non-normalized data).
seuObj <- CreateSeuratObject(counts = p56.p1p2cko.data, project = "NA_P56_P1P2CKO_CB")
print(dim(seuObj)) ## 21967 77749
...
##------------------------------------
## Save RData
save(seuObj, file = "SEURAT_NA_P56_P1P2CKO_CB.RData")
...
load("SEURAT_NA_P56_P1P2CKO_CB.RData")
# seuObj
...
save(seuObjDF, file = "SEURAT_NK_P56_P1P2CKO_CB_DF_TEMP.RData")


seuObjDF <- doubletFinder_v3(seuObjDF, PCs = 1:selpcs, pN = 0.25, pK = selpk, nExp = nExp_poi.adj, reuse.pANN = "pANN_0.25_0.005_7775", sct = FALSE)

table(seuObjDF$GenoAgeSample, seuObjDF$DF.classifications_0.25_0.005_7775)        
#                     Doublet Singlet
#   P1P2CKO_P56_NA02     1189    6964
#   P1P2CKO_P56_NA04A    1030   26407
#   P1P2CKO_P56_NA04B    5556   22980
#   P1P2CKO_P56_NA09        0   13623

table(seuObjDF$GenoAgeSample, seuObjDF$DF.classifications_0.25_0.005_7297)             
#                     Doublet Singlet
#   P1P2CKO_P56_NA02     1114    7039
#   P1P2CKO_P56_NA04A     937   26500
#   P1P2CKO_P56_NA04B    5246   23290
#   P1P2CKO_P56_NA09        0   13623


seuMeta <- as.data.frame(seuObjDF@meta.data)
seuMetaFiltTemp <- seuMeta[seuMeta$DF.classifications_0.25_0.005_7775 == "Singlet",]
seuMetaFilt <- seuMetaFiltTemp[seuMetaFiltTemp$DF.classifications_0.25_0.005_7297 == "Singlet",]
...
##------------------------------------
## Save RData
save(seuObjDF, file = "SEURAT_NA_P56_P1P2CKO_CB_DF.RData")
save(seuMeta, seuMetaFilt, file = "SEURAT_NA_P56_P1P2CKO_CB_DF_FILT_META.RData")
