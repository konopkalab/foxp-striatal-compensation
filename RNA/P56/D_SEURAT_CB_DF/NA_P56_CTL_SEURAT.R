...
# load("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P56/03_DOUBLETSFINDER/CTL/SEURAT_NA_P56_CTL_CB_DF.RData")
# # seuObjDF
...
# # CTL_P56_NA03A CTL_P56_NA03B CTL_P56_NA05A CTL_P56_NA05B  CTL_P56_NA07 
# #         24418         23659         24454         22906         22075

# ## cells retained
# cells2keepTemp <- row.names(seuObjDFCleaned@meta.data[seuObjDFCleaned@meta.data$DoubletFinder == "Retained",])
# cells2keepNA03A <- cells2keepTemp[grepl("^P56_CTL_NA03A_", cells2keepTemp)]
# cells2keepNA03B <- cells2keepTemp[grepl("^P56_CTL_NA03B_", cells2keepTemp)]
# cells2keepNA05A <- cells2keepTemp[grepl("^P56_CTL_NA05A_", cells2keepTemp)]
# cells2keepNA05B <- cells2keepTemp[grepl("^P56_CTL_NA05B_", cells2keepTemp)]
# cells2keepNA07 <- cells2keepTemp[grepl("^P56_CTL_NA07_", cells2keepTemp)]

# cells2keepNA03A <- gsub("P56_CTL_NA03A_", "P56_CTL_NA03_", cells2keepNA03A)
# cells2keepNA03B <- gsub("P56_CTL_NA03B_", "P56_CTL_NA03_", cells2keepNA03B)
# cells2keepNA05A <- gsub("P56_CTL_NA05A_", "P56_CTL_NA05_", cells2keepNA05A)
# cells2keepNA05B <- gsub("P56_CTL_NA05B_", "P56_CTL_NA05_", cells2keepNA05B)

# cells2keep <- c(unique(sort(c(cells2keepNA03A, cells2keepNA03B))), unique(sort(c(cells2keepNA05A, cells2keepNA05B))), cells2keepNA07)



# ## load dataset from 10X run
# ctl.n03a <- Read10X_h5("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P56/02_CELLBENDER/CTL_NA03_01/CellBender_Out_filtered.h5", use.names = TRUE, unique.features = TRUE)
# print(dim(ctl.n03a)) # 53715 24977

# ctl.n03b <- Read10X_h5("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P56/02_CELLBENDER/CTL_NA03_02/CellBender_Out_filtered.h5", use.names = TRUE, unique.features = TRUE)
# print(dim(ctl.n03b)) # 53715 28335

# ctl.n05a <- Read10X_h5("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P56/02_CELLBENDER/CTL_NA05_01/CellBender_Out_filtered.h5", use.names = TRUE, unique.features = TRUE)
# print(dim(ctl.n05a)) # 53715 24879

# ctl.n05b <- Read10X_h5("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P56/02_CELLBENDER/CTL_NA05_02/CellBender_Out_filtered.h5", use.names = TRUE, unique.features = TRUE)
# print(dim(ctl.n05b)) # 53715 26629

# ctl.n07 <- Read10X_h5("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P56/02_CELLBENDER/CTL_NA07/CellBender_Out_filtered.h5", use.names = TRUE, unique.features = TRUE)
# print(dim(ctl.n07)) # 53715 25749


# ## adding sample & condition info to column names
# colnames(ctl.n03a) <- paste("P56_CTL_NA03", colnames(ctl.n03a), sep = "_")
# colnames(ctl.n03b) <- paste("P56_CTL_NA03", colnames(ctl.n03b), sep = "_")
# colnames(ctl.n05a) <- paste("P56_CTL_NA05", colnames(ctl.n05a), sep = "_")
# colnames(ctl.n05b) <- paste("P56_CTL_NA05", colnames(ctl.n05b), sep = "_")
# colnames(ctl.n07) <- paste("P56_CTL_NA07", colnames(ctl.n07), sep = "_")


# ## fetch genes or rows corresponding to gencode protein coding gene symbols
# ctl.n03a.temp <- ctl.n03a[row.names(ctl.n03a) %in% ref$GeneSymbol,]
# ctl.n03b.temp <- ctl.n03b[row.names(ctl.n03b) %in% ref$GeneSymbol,]
# ctl.n05a.temp <- ctl.n05a[row.names(ctl.n05a) %in% ref$GeneSymbol,]
# ctl.n05b.temp <- ctl.n05b[row.names(ctl.n05b) %in% ref$GeneSymbol,]
# ctl.n07.temp <- ctl.n07[row.names(ctl.n07) %in% ref$GeneSymbol,]


# ## make a data from from matrix
# P56_CTL_NA03_A <- as.data.frame(as.matrix(ctl.n03a.temp))
# P56_CTL_NA03_B <- as.data.frame(as.matrix(ctl.n03b.temp))
# P56_CTL_NA05_A <- as.data.frame(as.matrix(ctl.n05a.temp))
# P56_CTL_NA05_B <- as.data.frame(as.matrix(ctl.n05b.temp))
# P56_CTL_NA07 <- as.data.frame(as.matrix(ctl.n07.temp))


# ## add gene symbol as a column
# P56_CTL_NA03_A$Genes <- row.names(P56_CTL_NA03_A)
# P56_CTL_NA03_B$Genes <- row.names(P56_CTL_NA03_B)
# P56_CTL_NA05_A$Genes <- row.names(P56_CTL_NA05_A)
# P56_CTL_NA05_B$Genes <- row.names(P56_CTL_NA05_B)
# P56_CTL_NA07$Genes <- row.names(P56_CTL_NA07)


# ## collapse the UMI counts for common cells by adding the counts for NA03
# commonNA03 <- intersect(colnames(P56_CTL_NA03_A), colnames(P56_CTL_NA03_B))
# P56_CTL_NA03_A_COMMON <- P56_CTL_NA03_A[,colnames(P56_CTL_NA03_A) %in% commonNA03]
# P56_CTL_NA03_B_COMMON <- P56_CTL_NA03_B[,colnames(P56_CTL_NA03_B) %in% commonNA03]

# dataCommonNA03 <- list("P56_CTL_NA03_A" = P56_CTL_NA03_A_COMMON, "P56_CTL_NA03_B" = P56_CTL_NA03_B_COMMON)

# combinedDataNA03 <- Reduce(function(x, y) { merge(x, y, all = TRUE, by = "Genes") } , dataCommonNA03)
# row.names(combinedDataNA03) <- combinedDataNA03$Genes
# combinedDataNA03$Genes <- NULL
# # colnames(combinedDataNA03) <- substr(colnames(combinedDataNA03), 1, nchar(colnames(combinedDataNA03)) - 2)

# combinedMetaNA03 <- as.data.frame(matrix(unlist(strsplit(colnames(combinedDataNA03), "_")), ncol = 4, byrow = TRUE))
# row.names(combinedMetaNA03) <- colnames(combinedDataNA03)
# colnames(combinedMetaNA03) <- c("Age", "Genotype", "Library", "CellBarcode")
# combinedMetaNA03$CellBarcode <- substr(combinedMetaNA03$CellBarcode, 1, nchar(combinedMetaNA03$CellBarcode) - 2)

# newDataTempNA03 <- as.data.frame(t(combinedDataNA03))
# newDataNA03 <- merge(newDataTempNA03, combinedMetaNA03, by = "row.names")
# row.names(newDataNA03) <- newDataNA03$Row.names
# newDataNA03$Row.names <- NULL
# cellBC <- list(newDataNA03$CellBarcode)
# newDataNA03$Genotype <- NULL
# newDataNA03$Library <- NULL
# newDataNA03$CellBarcode <- NULL
# newDataNA03$Age <- NULL

# newDataNA03AggrTemp <- aggregate(newDataNA03, by = cellBC, FUN = "sum")
# row.names(newDataNA03AggrTemp) <- newDataNA03AggrTemp$Group.1
# newDataNA03AggrTemp$Group.1 <- NULL

# P56_CTL_NA03 <- as.data.frame(t(newDataNA03AggrTemp))
# colnames(P56_CTL_NA03) <- paste("P56_CTL_NA03", colnames(P56_CTL_NA03), sep = "_")
# P56_CTL_NA03$Genes <- row.names(P56_CTL_NA03)




# ## collapse the UMI counts for common cells by adding the counts for NA05
# commonNA05 <- intersect(colnames(P56_CTL_NA05_A), colnames(P56_CTL_NA05_B))
# P56_CTL_NA05_A_COMMON <- P56_CTL_NA05_A[,colnames(P56_CTL_NA05_A) %in% commonNA05]
# P56_CTL_NA05_B_COMMON <- P56_CTL_NA05_B[,colnames(P56_CTL_NA05_B) %in% commonNA05]

# dataCommonNA05 <- list("P56_CTL_NA05_A" = P56_CTL_NA05_A_COMMON, "P56_CTL_NA05_B" = P56_CTL_NA05_B_COMMON)

# combinedDataNA05 <- Reduce(function(x, y) { merge(x, y, all = TRUE, by = "Genes") } , dataCommonNA05)
# row.names(combinedDataNA05) <- combinedDataNA05$Genes
# combinedDataNA05$Genes <- NULL
# # colnames(combinedDataNA05) <- substr(colnames(combinedDataNA05), 1, nchar(colnames(combinedDataNA05)) - 2)

# combinedMetaNA05 <- as.data.frame(matrix(unlist(strsplit(colnames(combinedDataNA05), "_")), ncol = 4, byrow = TRUE))
# row.names(combinedMetaNA05) <- colnames(combinedDataNA05)
# colnames(combinedMetaNA05) <- c("Age", "Genotype", "Library", "CellBarcode")
# combinedMetaNA05$CellBarcode <- substr(combinedMetaNA05$CellBarcode, 1, nchar(combinedMetaNA05$CellBarcode) - 2)

# newDataTempNA05 <- as.data.frame(t(combinedDataNA05))
# newDataNA05 <- merge(newDataTempNA05, combinedMetaNA05, by = "row.names")
# row.names(newDataNA05) <- newDataNA05$Row.names
# newDataNA05$Row.names <- NULL
# cellBC <- list(newDataNA05$CellBarcode)
# newDataNA05$Genotype <- NULL
# newDataNA05$Library <- NULL
# newDataNA05$CellBarcode <- NULL
# newDataNA05$Age <- NULL

# newDataNA05AggrTemp <- aggregate(newDataNA05, by = cellBC, FUN = "sum")
# row.names(newDataNA05AggrTemp) <- newDataNA05AggrTemp$Group.1
# newDataNA05AggrTemp$Group.1 <- NULL

# P56_CTL_NA05 <- as.data.frame(t(newDataNA05AggrTemp))
# colnames(P56_CTL_NA05) <- paste("P56_CTL_NA05", colnames(P56_CTL_NA05), sep = "_")
# P56_CTL_NA05$Genes <- row.names(P56_CTL_NA05)



# ## combine individual tables into a giant data frame
# dataCombined <- list("P56_CTL_NA03" = P56_CTL_NA03, "P56_CTL_NA05" = P56_CTL_NA05, "P56_CTL_NA07" = P56_CTL_NA07)
...
# print(dim(na.data)) ## 21967 75559

# p56.ctl.data <- na.data[,colnames(na.data) %in% cells2keep]
# print(dim(p56.ctl.data))
# # 21967 70907


# save(p56.ctl.data, file = "NA_SEURAT_CTL_P56_CB_DF_COUNTS.RData")



##------------------------------------
rm(list = ls())
load("NA_SEURAT_CTL_P56_CB_DF_COUNTS.RData")
# p56.ctl.data

## Initialize the Seurat object with the raw (non-normalized data).
seuObj <- CreateSeuratObject(counts = p56.ctl.data, project = "NA_CTL_P56")
print(dim(seuObj)) ## 21967 70907
...
seuObj$LibPrep <- gsub("P56_CTL_NA03", 1, seuObj$GenoAgeSample)
seuObj$LibPrep <- gsub("P56_CTL_NA05", 2, seuObj$LibPrep)
seuObj$LibPrep <- gsub("P56_CTL_NA07", 3, seuObj$LibPrep)

seuObj$SeqBatch <- gsub("P56_CTL_NA03", 1, seuObj$GenoAgeSample)
seuObj$SeqBatch <- gsub("P56_CTL_NA05", 1, seuObj$SeqBatch)
seuObj$SeqBatch <- gsub("P56_CTL_NA07", 2, seuObj$SeqBatch)

seuObj$Sex <- gsub("P56_CTL_NA03", "F", seuObj$GenoAgeSample)
seuObj$Sex <- gsub("P56_CTL_NA05", "F", seuObj$Sex)
seuObj$Sex <- gsub("P56_CTL_NA07", "F", seuObj$Sex)
...
save(seuObj, file = "NA_SEURAT_CTL_P56_TEMP.RData")


##------------------------------------
## Harmony
seuObj <- RunHarmony(object = seuObj, group.by.vars = c("LibPrep", "SeqBatch"), reduction = "pca", assay.use = "RNA") #, project.dim = FALSE)
...
##------------------------------------
## Save RData
save(seuObj, file = "NA_SEURAT_CTL_P56.RData")



HARMONISED_UMAP <- DimPlot(seuObj, group.by = c("GenoAgeSample", "LibPrep", "SeqBatch", "Sex", "RNA_snn_res.0.8"), pt.size = 0.01)
ggsave("NA_SEURAT_CTL_P56_UMAP.pdf", plot = HARMONISED_UMAP, width = 20, height = 12, units = "in", dpi = 300)

HARMONISED_UMAP_FACET <- DimPlot(seuObj, group.by = "GenoAgeSample", pt.size = 0.01, split.by = "GenoAgeSample", ncol = 3)
ggsave("NA_SEURAT_CTL_P56_UMAP_FACET.pdf", plot = HARMONISED_UMAP_FACET, width = 20, height = 6, units = "in", dpi = 300)
...
# save(seuObj, file = "NA_SEURAT_CTL_P1_UPDATED.RData")
...
# load("NA_SEURAT_CTL_P1_UPDATED.RData")
# # seuObj
...
# save(seuObj, file = "NA_SEURAT_CTL_P1_UPDATED_WPRE_NEW.RData")
...
# # save(seuObj, file = "NA_SEURAT_CTL_P1_UPDATED_WPRE.RData")
