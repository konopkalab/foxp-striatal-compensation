...
# load("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P56/03_DOUBLETSFINDER/P1CKO/SEURAT_NA_P56_P1CKO_CB_DF.RData")
# # seuObjDF
...
# # P1CKO_P56_NA01A P1CKO_P56_NA01B  P1CKO_P56_NA06  P1CKO_P56_NA10 
# #           19934           17885           26215            9053

# ## cells retained
# cells2keepTemp <- row.names(seuObjDFCleaned@meta.data[seuObjDFCleaned@meta.data$DoubletFinder == "Retained",])
# cells2keepNA01A <- cells2keepTemp[grepl("^P56_P1CKO_NA01A_", cells2keepTemp)]
# cells2keepNA01B <- cells2keepTemp[grepl("^P56_P1CKO_NA01B_", cells2keepTemp)]
# cells2keepNA06 <- cells2keepTemp[grepl("^P56_P1CKO_NA06_", cells2keepTemp)]
# cells2keepNA10 <- cells2keepTemp[grepl("^P56_P1CKO_NA10_", cells2keepTemp)]

# cells2keepNA01A <- gsub("P56_P1CKO_NA01A_", "P56_P1CKO_NA01_", cells2keepNA01A)
# cells2keepNA01B <- gsub("P56_P1CKO_NA01B_", "P56_P1CKO_NA01_", cells2keepNA01B)

# cells2keep <- c(unique(sort(c(cells2keepNA01A, cells2keepNA01B))), cells2keepNA06, cells2keepNA10)



# ## load dataset from 10X run
# p1cko.na01a <- Read10X_h5("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P56/02_CELLBENDER/P1CKO_NA01_01/CellBender_Out_filtered.h5", use.names = TRUE, unique.features = TRUE)
# print(dim(p1cko.na01a)) # 53715 20938

# p1cko.na01b <- Read10X_h5("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P56/02_CELLBENDER/P1CKO_NA01_02/CellBender_Out_filtered.h5", use.names = TRUE, unique.features = TRUE)
# print(dim(p1cko.na01b)) # 53715 22182

# p1cko.na06 <- Read10X_h5("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P56/02_CELLBENDER/P1CKO_NA06/CellBender_Out_filtered.h5", use.names = TRUE, unique.features = TRUE)
# print(dim(p1cko.na06)) # 53715 26659

# p1cko.na10 <- Read10X_h5("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P56/02_CELLBENDER/P1CKO_NA10/CellBender_Out_filtered.h5", use.names = TRUE, unique.features = TRUE)
# print(dim(p1cko.na10)) # 53715 11429


# ## adding sample & condition info to column names
# colnames(p1cko.na01a) <- paste("P56_P1CKO_NA01", colnames(p1cko.na01a), sep = "_")
# colnames(p1cko.na01b) <- paste("P56_P1CKO_NA01", colnames(p1cko.na01b), sep = "_")
# colnames(p1cko.na06) <- paste("P56_P1CKO_NA06", colnames(p1cko.na06), sep = "_")
# colnames(p1cko.na10) <- paste("P56_P1CKO_NA10", colnames(p1cko.na10), sep = "_")


# ## fetch genes or rows corresponding to gencode protein coding gene symbols
# p1cko.na01a.temp <- p1cko.na01a[row.names(p1cko.na01a) %in% ref$GeneSymbol,]
# p1cko.na01b.temp <- p1cko.na01b[row.names(p1cko.na01b) %in% ref$GeneSymbol,]
# p1cko.na06.temp <- p1cko.na06[row.names(p1cko.na06) %in% ref$GeneSymbol,]
# p1cko.na10.temp <- p1cko.na10[row.names(p1cko.na10) %in% ref$GeneSymbol,]


# ## make a data from from matrix
# P56_P1CKO_NA01_A <- as.data.frame(as.matrix(p1cko.na01a.temp))
# P56_P1CKO_NA01_B <- as.data.frame(as.matrix(p1cko.na01b.temp))
# P56_P1CKO_NA06 <- as.data.frame(as.matrix(p1cko.na06.temp))
# P56_P1CKO_NA10 <- as.data.frame(as.matrix(p1cko.na10.temp))


# ## add gene symbol as a column
# P56_P1CKO_NA01_A$Genes <- row.names(P56_P1CKO_NA01_A)
# P56_P1CKO_NA01_B$Genes <- row.names(P56_P1CKO_NA01_B)
# P56_P1CKO_NA06$Genes <- row.names(P56_P1CKO_NA06)
# P56_P1CKO_NA10$Genes <- row.names(P56_P1CKO_NA10)



# ## collapse the UMI counts for common cells by adding the counts for NA01
# commonNA01 <- intersect(colnames(P56_P1CKO_NA01_A), colnames(P56_P1CKO_NA01_B))
# P56_P1CKO_NA01_A_COMMON <- P56_P1CKO_NA01_A[,colnames(P56_P1CKO_NA01_A) %in% commonNA01]
# P56_P1CKO_NA01_B_COMMON <- P56_P1CKO_NA01_B[,colnames(P56_P1CKO_NA01_B) %in% commonNA01]

# dataCommonNA01 <- list("P56_P1CKO_NA01_A" = P56_P1CKO_NA01_A_COMMON, "P56_P1CKO_NA01_B" = P56_P1CKO_NA01_B_COMMON)

# combinedDataNA01 <- Reduce(function(x, y) { merge(x, y, all = TRUE, by = "Genes") } , dataCommonNA01)
# row.names(combinedDataNA01) <- combinedDataNA01$Genes
# combinedDataNA01$Genes <- NULL
# # colnames(combinedDataNA01) <- substr(colnames(combinedDataNA01), 1, nchar(colnames(combinedDataNA01)) - 2)

# combinedMetaNA01 <- as.data.frame(matrix(unlist(strsplit(colnames(combinedDataNA01), "_")), ncol = 4, byrow = TRUE))
# row.names(combinedMetaNA01) <- colnames(combinedDataNA01)
# colnames(combinedMetaNA01) <- c("Age", "Genotype", "Library", "CellBarcode")
# combinedMetaNA01$CellBarcode <- substr(combinedMetaNA01$CellBarcode, 1, nchar(combinedMetaNA01$CellBarcode) - 2)

# newDataTempNA01 <- as.data.frame(t(combinedDataNA01))
# newDataNA01 <- merge(newDataTempNA01, combinedMetaNA01, by = "row.names")
# row.names(newDataNA01) <- newDataNA01$Row.names
# newDataNA01$Row.names <- NULL
# cellBC <- list(newDataNA01$CellBarcode)
# newDataNA01$Genotype <- NULL
# newDataNA01$Library <- NULL
# newDataNA01$CellBarcode <- NULL
# newDataNA01$Age <- NULL

# newDataNA01AggrTemp <- aggregate(newDataNA01, by = cellBC, FUN = "sum")
# row.names(newDataNA01AggrTemp) <- newDataNA01AggrTemp$Group.1
# newDataNA01AggrTemp$Group.1 <- NULL

# P56_P1CKO_NA01 <- as.data.frame(t(newDataNA01AggrTemp))
# colnames(P56_P1CKO_NA01) <- paste("P56_P1CKO_NA01", colnames(P56_P1CKO_NA01), sep = "_")
# P56_P1CKO_NA01$Genes <- row.names(P56_P1CKO_NA01)



# ## combine individual tables into a giant data frame
# dataCombined <- list("P56_P1CKO_NA01" = P56_P1CKO_NA01, "P56_P1CKO_NA06" = P56_P1CKO_NA06, "P56_P1CKO_NA10" = P56_P1CKO_NA10)
...
# p56.p1cko.data <- na.data[,colnames(na.data) %in% cells2keep]
# print(dim(p56.p1cko.data))
# # 21967 55192


# save(p56.p1cko.data, file = "NA_SEURAT_P1CKO_P56_CB_DF_COUNTS.RData")



##------------------------------------
rm(list = ls())
load("NA_SEURAT_P1CKO_P56_CB_DF_COUNTS.RData")
# p56.p1cko.data

## Initialize the Seurat object with the raw (non-normalized data).
seuObj <- CreateSeuratObject(counts = p56.p1cko.data, project = "NA_P1CKO_P56")
print(dim(seuObj)) ## 21967 55192
...
seuObj$LibPrep <- gsub("P56_P1CKO_NA01", 1, seuObj$GenoAgeSample)
seuObj$LibPrep <- gsub("P56_P1CKO_NA06", 3, seuObj$LibPrep)
seuObj$LibPrep <- gsub("P56_P1CKO_NA10", 4, seuObj$LibPrep)

seuObj$SeqBatch <- gsub("P56_P1CKO_NA01", 1, seuObj$GenoAgeSample)
seuObj$SeqBatch <- gsub("P56_P1CKO_NA06", 2, seuObj$SeqBatch)
seuObj$SeqBatch <- gsub("P56_P1CKO_NA10", 3, seuObj$SeqBatch)

seuObj$Sex <- gsub("P56_P1CKO_NA01", "M", seuObj$GenoAgeSample)
seuObj$Sex <- gsub("P56_P1CKO_NA06", "M", seuObj$Sex)
seuObj$Sex <- gsub("P56_P1CKO_NA10", "M", seuObj$Sex)
...
# P56_P1CKO_NA01A P56_P1CKO_NA01B  P56_P1CKO_NA06  P56_P1CKO_NA10 
#           19934           17885           26215            9053
...
save(seuObj, file = "NA_SEURAT_P1CKO_P56_TEMP.RData")
...
##------------------------------------
## Save RData
save(seuObj, file = "NA_SEURAT_P1CKO_P56.RData")



HARMONISED_UMAP <- DimPlot(seuObj, group.by = c("GenoAgeSample", "LibPrep", "SeqBatch", "Sex", "RNA_snn_res.0.8"), pt.size = 0.01)
ggsave("NA_SEURAT_P1CKO_P56_UMAP.pdf", plot = HARMONISED_UMAP, width = 20, height = 12, units = "in", dpi = 300)

HARMONISED_UMAP_FACET <- DimPlot(seuObj, group.by = "GenoAgeSample", pt.size = 0.01, split.by = "GenoAgeSample", ncol = 3)
ggsave("NA_SEURAT_P1CKO_P56_UMAP_FACET.pdf", plot = HARMONISED_UMAP_FACET, width = 20, height = 6, units = "in", dpi = 300)
...
# save(seuObj, file = "NA_SEURAT_P1CKO_P1_UPDATED.RData")
...
# load("NA_SEURAT_P1CKO_P1_UPDATED.RData")
# # seuObj

# seuMetaTemp <- as.data.frame(seuObj@meta.data)

# ## Load P1-CTL-01 and P1-CTL-02 from 10X run## load dataset from 10X run
# p1.p1cko.1 <- Read10X(data.dir = "/work/Neuroinformatics_Core/s175848/DATA_BACKUP_AK/NOVASEQ_JUN2021/03_COUNT_WT-WPRE-5/WT03WPRE/outs/filtered_feature_bc_matrix")
# print(dim(p1.p1cko.1)) # 53716 23368

# p1.p1cko.2 <- Read10X(data.dir = "/work/Neuroinformatics_Core/s175848/DATA_BACKUP_AK/NOVASEQ_JUN2021/03_COUNT_WT-WPRE-5/WT04WPRE/outs/filtered_feature_bc_matrix")
# print(dim(p1.p1cko.2)) # 53716 14099

# ## adding sample & condition info to column names
# colnames(p1.p1cko.1) <- paste("CTL_P1_01", colnames(p1.p1cko.1), sep = "_")
# colnames(p1.p1cko.2) <- paste("CTL_P1_02", colnames(p1.p1cko.2), sep = "_")

# p1.ctlw.1 <- as.data.frame(p1.p1cko.1["WPRE",])
# p1.ctlw.2 <- as.data.frame(p1.p1cko.2["WPRE",])
...
# save(seuObj, file = "NA_SEURAT_P1CKO_P1_UPDATED_WPRE_NEW.RData")
...
# # save(seuObj, file = "NA_SEURAT_P1CKO_P1_UPDATED_WPRE.RData")
