...
print("P56 P2CKO")
load("/work/Neuroinformatics_Core/akulk1/CELLRANGER/NA_DATA/P56/12_P2CKO_woAmbient_woDoublets/NA_SEURAT_P2CKO_P56.RData")
# seuObj
p2cko.seuObj <- seuObj
rm(seuObj)
dim(p2cko.seuObj)
...
##-------------------------------------------------------
## QC PLOTS FOR CTL
Idents(ctl.seuObj) <- "GenoAgeSample"
ctl.qcplot1 <- FeatureScatter(ctl.seuObj, feature1 = "nCount_RNA", feature2 = "pMito_RNA") + geom_hline(yintercept = pmhi, linetype="dashed") + geom_vline(xintercept = nuhi, linetype="dashed")
ctl.qcplot2 <- FeatureScatter(ctl.seuObj, feature1 = "nCount_RNA", feature2 = "nFeature_RNA") + geom_hline(yintercept = (0.2 * ncol(ctl.seuObj)) / 100, linetype="dashed") + geom_vline(xintercept = nuhi, linetype="dashed")
ctl.qcplots <- grid.arrange(ctl.qcplot1, ctl.qcplot2, ncol = 2)
ggsave(filename = "CTL_QC_PLOT_BEFORE.pdf", plot = ctl.qcplots, width = 15, height = 6, dpi = 150, units = "in")

ctl.seuObj$percent_mito <- ctl.seuObj$pMito_RNA
ctl.qcplot4 <- QC_Plots_UMIs(seurat_object = ctl.seuObj, high_cutoff = nuhi, pt.size = 0)
ctl.qcplot5 <- QC_Plots_Genes(seurat_object = ctl.seuObj, low_cutoff = 0.2 * ncol(ctl.seuObj) / 100, pt.size = 0)
ctl.qcplot6 <- QC_Plots_Mito(seurat_object = ctl.seuObj, high_cutoff = pmhi, pt.size = 0)
ctl.qcplotsb <- grid.arrange(ctl.qcplot4, ctl.qcplot5, ctl.qcplot6, ncol = 3)
ggsave(filename = "CTL_QC_PLOT_BEFORE2.pdf", plot = ctl.qcplotsb, width = 12, height = 6, dpi = 150, units = "in")


##-------------------------------------------------------
## QC PLOTS FOR P1CKO
Idents(p1cko.seuObj) <- "GenoAgeSample"
p1cko.qcplot1 <- FeatureScatter(p1cko.seuObj, feature1 = "nCount_RNA", feature2 = "pMito_RNA") + geom_hline(yintercept = pmhi, linetype="dashed") + geom_vline(xintercept = nuhi, linetype="dashed")
p1cko.qcplot2 <- FeatureScatter(p1cko.seuObj, feature1 = "nCount_RNA", feature2 = "nFeature_RNA") + geom_hline(yintercept = (0.2 * ncol(p1cko.seuObj)) / 100, linetype="dashed") + geom_vline(xintercept = nuhi, linetype="dashed")
p1cko.qcplots <- grid.arrange(p1cko.qcplot1, p1cko.qcplot2, ncol = 2)
ggsave(filename = "P1CKO_QC_PLOT_BEFORE.pdf", plot = p1cko.qcplots, width = 15, height = 6, dpi = 150, units = "in")

p1cko.seuObj$percent_mito <- p1cko.seuObj$pMito_RNA
p1cko.qcplot4 <- QC_Plots_UMIs(seurat_object = p1cko.seuObj, high_cutoff = nuhi, pt.size = 0)
p1cko.qcplot5 <- QC_Plots_Genes(seurat_object = p1cko.seuObj, low_cutoff = 0.2 * ncol(p1cko.seuObj) / 100, pt.size = 0)
p1cko.qcplot6 <- QC_Plots_Mito(seurat_object = p1cko.seuObj, high_cutoff = pmhi, pt.size = 0)
p1cko.qcplotsb <- grid.arrange(p1cko.qcplot4, p1cko.qcplot5, p1cko.qcplot6, ncol = 3)
ggsave(filename = "P1CKO_QC_PLOT_BEFORE2.pdf", plot = p1cko.qcplotsb, width = 12, height = 6, dpi = 150, units = "in")


##-------------------------------------------------------
## QC PLOTS FOR P2CKO
Idents(p2cko.seuObj) <- "GenoAgeSample"
p2cko.qcplot1 <- FeatureScatter(p2cko.seuObj, feature1 = "nCount_RNA", feature2 = "pMito_RNA") + geom_hline(yintercept = pmhi, linetype="dashed") + geom_vline(xintercept = nuhi, linetype="dashed")
p2cko.qcplot2 <- FeatureScatter(p2cko.seuObj, feature1 = "nCount_RNA", feature2 = "nFeature_RNA") + geom_hline(yintercept = (0.2 * ncol(p2cko.seuObj)) / 100, linetype="dashed") + geom_vline(xintercept = nuhi, linetype="dashed")
p2cko.qcplots <- grid.arrange(p2cko.qcplot1, p2cko.qcplot2, ncol = 2)
ggsave(filename = "P2CKO_QC_PLOT_BEFORE.pdf", plot = p2cko.qcplots, width = 15, height = 6, dpi = 150, units = "in")

p2cko.seuObj$percent_mito <- p2cko.seuObj$pMito_RNA
p2cko.qcplot4 <- QC_Plots_UMIs(seurat_object = p2cko.seuObj, high_cutoff = nuhi, pt.size = 0)
p2cko.qcplot5 <- QC_Plots_Genes(seurat_object = p2cko.seuObj, low_cutoff = 0.2 * ncol(p2cko.seuObj) / 100, pt.size = 0)
p2cko.qcplot6 <- QC_Plots_Mito(seurat_object = p2cko.seuObj, high_cutoff = pmhi, pt.size = 0)
p2cko.qcplotsb <- grid.arrange(p2cko.qcplot4, p2cko.qcplot5, p2cko.qcplot6, ncol = 3)
ggsave(filename = "P2CKO_QC_PLOT_BEFORE2.pdf", plot = p2cko.qcplotsb, width = 12, height = 6, dpi = 150, units = "in")


##-------------------------------------------------------
## QC PLOTS FOR P1P2CKO
Idents(p1p2cko.seuObj) <- "GenoAgeSample"
p1p2cko.qcplot1 <- FeatureScatter(p1p2cko.seuObj, feature1 = "nCount_RNA", feature2 = "pMito_RNA") + geom_hline(yintercept = pmhi, linetype="dashed") + geom_vline(xintercept = nuhi, linetype="dashed")
p1p2cko.qcplot2 <- FeatureScatter(p1p2cko.seuObj, feature1 = "nCount_RNA", feature2 = "nFeature_RNA") + geom_hline(yintercept = (0.2 * ncol(p1p2cko.seuObj)) / 100, linetype="dashed") + geom_vline(xintercept = nuhi, linetype="dashed")
p1p2cko.qcplots <- grid.arrange(p1p2cko.qcplot1, p1p2cko.qcplot2, ncol = 2)
ggsave(filename = "P1P2CKO_QC_PLOT_BEFORE.pdf", plot = p1p2cko.qcplots, width = 15, height = 6, dpi = 150, units = "in")

p1p2cko.seuObj$percent_mito <- p1p2cko.seuObj$pMito_RNA
p1p2cko.qcplot4 <- QC_Plots_UMIs(seurat_object = p1p2cko.seuObj, high_cutoff = nuhi, pt.size = 0)
p1p2cko.qcplot5 <- QC_Plots_Genes(seurat_object = p1p2cko.seuObj, low_cutoff = 0.2 * ncol(p1p2cko.seuObj) / 100, pt.size = 0)
p1p2cko.qcplot6 <- QC_Plots_Mito(seurat_object = p1p2cko.seuObj, high_cutoff = pmhi, pt.size = 0)
p1p2cko.qcplotsb <- grid.arrange(p1p2cko.qcplot4, p1p2cko.qcplot5, p1p2cko.qcplot6, ncol = 3)
ggsave(filename = "P1P2CKO_QC_PLOT_BEFORE2.pdf", plot = p1p2cko.qcplotsb, width = 12, height = 6, dpi = 150, units = "in")
...
seuObjFilt$CellType <- gsub("^11$", "A_PROG", seuObjFilt$RNA_snn_res.1.2)
seuObjFilt$CellType <- gsub("^0$|^2$", "B_dSPN_iSPN", seuObjFilt$CellType)
seuObjFilt$CellType <- gsub("^12$|^16$", "C_eSPN", seuObjFilt$CellType)
seuObjFilt$CellType <- gsub("^25$|^27$|^31$|^42$", "D_Other_SPN", seuObjFilt$CellType)
seuObjFilt$CellType <- gsub("^9$|^21$|^13$|^4$|^18$|^22$|^41$|^28$|^49$", "E_Other_SPN", seuObjFilt$CellType)
seuObjFilt$CellType <- gsub("^3$|^5$|^52$|^29$", "F_ASTRO", seuObjFilt$CellType)
seuObjFilt$CellType <- gsub("^1$|^53$|^35$|^43$|^38$|^46$|^51$|^14$|^47$", "G_OLIGO", seuObjFilt$CellType)
seuObjFilt$CellType <- gsub("^19$|^51$", "H_MICRO", seuObjFilt$CellType)
seuObjFilt$CellType <- gsub("^20$|^40$|^48$|^26$|^37$", "I_ENDO_EPEN", seuObjFilt$CellType)
seuObjFilt$CellType <- gsub("^36$|^6$|^8$|^33$|^10$|^17$|^30$|^15$|^39$|^24$|^32$|^7$|^54$", "J_CORTICAL", seuObjFilt$CellType)
seuObjFilt$CellType <- gsub("^23$|^34$|^50$|^44$|^45$", "K_UNDET", seuObjFilt$CellType)
...
p56_all_celltype_sample <- as.data.frame.matrix(table(seuObjFilt$CellType, seuObjFilt$GenoAgeSample))
write.table(p56_all_celltype_sample, "P56_ALL_CELLTYPE_BY_SAMPLE.txt", row.names = TRUE, col.names = TRUE, quote = FALSE, sep = "\t")

p56_all_celltypecluster_sample <- as.data.frame.matrix(table(seuObjFilt$CellTypeCluster, seuObjFilt$GenoAgeSample))
write.table(p56_all_celltypecluster_sample, "P56_ALL_CELLTYPE_CLUSTER_BY_SAMPLE.txt", row.names = TRUE, col.names = TRUE, quote = FALSE, sep = "\t")


seuObjSel <- subset(seuObjFilt, idents = "K_UNDET", invert = TRUE)

plotCluUMAP1 <- DimPlot(seuObjFilt, group.by = "CellType", pt.size = 0.1, reduction = "umap", label = FALSE, raster = TRUE)
ggsave(filename = "NA_SEURAT_UMAP_CELLTYPE.pdf", plot = plotCluUMAP1, width = 7, height = 6, units = "in", dpi = 300)

plotCluUMAP2 <- DimPlot(seuObjSel, group.by = "CellTypeCluster", pt.size = 0.1, reduction = "umap", label = TRUE, label.size = 2, raster = TRUE)
ggsave(filename = "NA_SEURAT_UMAP_CELLTYPE_CLUSTER_LABELLED.pdf", plot = plotCluUMAP2, width = 14, height = 8, units = "in", dpi = 300)

plotCluUMAP2b <- DimPlot(seuObjSel, group.by = "CellTypeCluster", pt.size = 0.1, reduction = "umap", label = FALSE, label.size = 2, raster = TRUE)
ggsave(filename = "NA_SEURAT_UMAP_CELLTYPE_CLUSTER.pdf", plot = plotCluUMAP2b, width = 14, height = 8, units = "in", dpi = 300)
...
# # P56_P1P2CKO_NA45   P56_P2CKO_NA25   P56_P2CKO_NA44 
# #            15837            13615            24533
...
# ##-------------------------------------------------------
# ## HARMONY
# seuObjFilt <- RunHarmony(seuObjFilt, group.by.vars = c("LibPrep", "SeqBatch", "Sex"))
...
p7 <- ggplot(cellsPerCluster2) +
        ...
        theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) + 
        NULL
ggsave(filename = paste("NA_SEURAT_GENOAGESAMPLE_PER_CLUSTER_1.2.pdf", sep = "_"), plot = p7, width = 9, height = 4, units = "in", dpi = 150)
...
ggsave(filename = "NA_SEURAT_PLOT_ViolinPlot_1.2_C.pdf", plot = vpl3 , width = 12, height = 5, units = "in", dpi = 150, useDingbats=FALSE)
