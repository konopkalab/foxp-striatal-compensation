...
## Are motifs enriched in open regions and closed regions inversely correlated?
enrichedMotifsCTLP1CKOdn_reordered <- enrichedMotifsCTLP1CKOdn[match(enrichedMotifsCTLP1CKOup$motif.name, enrichedMotifsCTLP1CKOdn$motif.name),]
enrichedMotifsCTLP2CKOdn_reordered <- enrichedMotifsCTLP2CKOdn[match(enrichedMotifsCTLP2CKOup$motif.name, enrichedMotifsCTLP2CKOdn$motif.name),]
enrichedMotifsCTLP1P2CKOdn_reordered <- enrichedMotifsCTLP1P2CKOdn[match(enrichedMotifsCTLP1P2CKOup$motif.name, enrichedMotifsCTLP1P2CKOdn$motif.name),]

## Find pearson correlation coefficient
ctl_p1cko_cor <- cor.test(enrichedMotifsCTLP1CKOup$fold.enrichment, enrichedMotifsCTLP1CKOdn_reordered$fold.enrichment)
ctl_p2cko_cor <- cor.test(enrichedMotifsCTLP2CKOup$fold.enrichment, enrichedMotifsCTLP2CKOdn_reordered$fold.enrichment)
ctl_p1p2cko_cor <- cor.test(enrichedMotifsCTLP1P2CKOup$fold.enrichment, enrichedMotifsCTLP1P2CKOdn_reordered$fold.enrichment)



## Plot fold changes and correlation
pdf("NA_ATAC_CTL_P1CKO_MOTIF_CORR.pdf")
par(mar = c(5, 6, 2, 2))
plot(enrichedMotifsCTLP1CKOup$fold.enrichment, enrichedMotifsCTLP1CKOdn_reordered$fold.enrichment,
	xlab = 'Open in P1CKO vs. CTL\nMotif Enrichment Fold Change',
	ylab = 'Closed in P1CKO vs. CTL\nMotif Enrichment Fold Change',
    xlim = c(0, 4), 
    ylim = c(0, 4),
    pch = 20)
text(3.5, 3.5, paste("Corr: ", sprintf("%0.3f", ctl_p1cko_cor$estimate), "\nP-val: ", sprintf("%0.5f", ctl_p1cko_cor$p.value), sep = ""))
dev.off()


pdf("NA_ATAC_CTL_P2CKO_MOTIF_CORR.pdf")
par(mar = c(5, 6, 2, 2))
plot(enrichedMotifsCTLP2CKOup$fold.enrichment, enrichedMotifsCTLP2CKOdn_reordered$fold.enrichment,
	xlab = 'Open in P2CKO vs. CTL\nMotif Enrichment Fold Change',
	ylab = 'Closed in P2CKO vs. CTL\nMotif Enrichment Fold Change',
    xlim = c(0, 4), 
    ylim = c(0, 4),
    pch = 20)
text(3.5, 3.5, paste("Corr: ", sprintf("%0.3f", ctl_p2cko_cor$estimate), "\nP-val: ", sprintf("%0.5f", ctl_p2cko_cor$p.value), sep = ""))
dev.off()


pdf("NA_ATAC_CTL_P1P2CKO_MOTIF_CORR.pdf")
par(mar = c(5, 6, 2, 2))
plot(enrichedMotifsCTLP1P2CKOup$fold.enrichment, enrichedMotifsCTLP1P2CKOdn_reordered$fold.enrichment,
	xlab = 'Open in P1P2CKO vs. CTL\nMotif Enrichment Fold Change',
	ylab = 'Closed in P1P2CKO vs. CTL\nMotif Enrichment Fold Change',
    xlim = c(0, 4), 
    ylim = c(0, 4),
    pch = 20)
text(3.5, 3.5, paste("Corr: ", sprintf("%0.3f", ctl_p1p2cko_cor$estimate), "\nP-val: ", sprintf("%0.5f", ctl_p1p2cko_cor$p.value), sep = ""))    
dev.off()
...
# # Are motifs enriched in open regions and closed regions inversely correlated?
# enriched_motifs_closed_reordered = enriched_motifs_closed[match(enriched_motifs_open$motif.name, enriched_motifs_closed$motif.name),]

# # Find pearson correlation coefficient
# cor.test(enriched_motifs_open$fold.enrichment, enriched_motifs_closed_reordered$fold.enrichment)

# # Plot the fold changes
# pdf("Motif_correlation_ecig.pdf")
# par(mar = c(5, 5, 5, 5))
# plot(enriched_motifs_open$fold.enrichment, enriched_motifs_closed_reordered$fold.enrichment,
# 	xlab = 'Opened After Ecigarette\nMotif Enrichment Fold Change',
# 	ylab = 'Closed After Ecigarette\nMotif Enrichment Fold Change')
# dev.off()

# # Motif enrichment suggests that the targets of transcription factors enriched for neuroblasts identity (e.g Dlx1) are enriched among the peaks that are closed after e-cigarette exposure.

# # Are neuroblasts relatively depleted after e-cigarette exposure? We can use single-cell data to answer this question too.

# # Extract meta data
# atacMeta = seurat_atac@meta.data

# # Proportion of from control and ecigarette among neuroblasts
# atacMeta[atacMeta$newannot == 'Neuroblasts', 'Condition'] %>% table %>% prop.table

# # Proportion of cells from control and ecigarette among all cells
# atacMeta[, 'Condition'] %>% table %>% prop.table
