...
nuclei.by.genosample <- as.data.frame(table(metaatac$Dataset))
colnames(nuclei.by.genosample) <- c("Sample", "Nuclei")
nuclei.by.genosample$Genotype <- gsub("^NA104_CTL$|^NA108_CTL$|^NA112_CTL$", "A_CTL", nuclei.by.genosample$Sample)
nuclei.by.genosample$Genotype <- gsub("^NA105_P1CKO$|^NA109_P1CKO$|^NA113_P1CKO$", "B_P1CKO", nuclei.by.genosample$Genotype)
nuclei.by.genosample$Genotype <- gsub("^NA106_P2CKO$|^NA110_P2CKO$|^NA114_P2CKO$", "C_P2CKO", nuclei.by.genosample$Genotype)
nuclei.by.genosample$Genotype <- gsub("^NA107_P1P2CKO$|^NA111_P1P2CKO$|^NA115_P1P2CKO$", "D_P1P2CKO", nuclei.by.genosample$Genotype)

row.names(nuclei.by.genosample) <- nuclei.by.genosample$Sample

nuclei.by.genosample$Sample <- gsub("_CTL$|_P1CKO$|_P2CKO$|_P1P2CKO$", "", row.names(nuclei.by.genosample))

nuclei.by.genosample$GenoSample <- paste(nuclei.by.genosample$Genotype, nuclei.by.genosample$Sample, sep = "_")


## Barplot for Number of Nuclei
p1 <- ggplot(nuclei.by.genosample, aes(x = GenoSample, y = Nuclei, fill = Genotype)) +
      geom_bar(stat = "identity", position = "dodge") +
      theme_classic() +
      theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
      scale_fill_manual(values = c("#191919", "#BB0BF8", "#5DCA9F", "#EEBC10")) +
      labs(x = "Samples", y = "# Nuclei", fill = "Genotype") +
      ggtitle("Number of Nuclei") + 
      NULL
ggsave(filename = "NA_ATAC_NUCLEI_PER_SAMPLE_BARPLOT.pdf", plot = p1, width = 6, height = 4, units = "in", dpi = 300)



##---------------------------------------------------------------------------##
## Boxplot for Number of Peaks
p2 <- ggplot(metaatac, aes(x = GenoSample, y = nCount_peaks, color = Genotype)) +
      geom_boxplot() +
      theme_classic() +
      theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
      scale_color_manual(values = c("#191919", "#BB0BF8", "#5DCA9F", "#EEBC10")) +
      labs(x = "Samples", y = "# Peaks", fill = "Genotype") +
      ggtitle("Number of Peaks") +
      NULL
ggsave(filename = "NA_ATAC_PEAKS_PER_SAMPLE_BOXPLOT.pdf", plot = p2, width = 6, height = 4, units = "in", dpi = 300)



##---------------------------------------------------------------------------##
## Boxplot for Number Reads in Peaks
p3 <- ggplot(metaatac, aes(x = GenoSample, y = peak_region_fragments, color = Genotype)) +
      geom_boxplot() +
      theme_classic() +
      theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
      scale_color_manual(values = c("#191919", "#BB0BF8", "#5DCA9F", "#EEBC10")) +
      ...
      NULL
ggsave(filename = "NA_ATAC_READS_IN_PEAKS_PER_SAMPLE_BOXPLOT.pdf", plot = p3, width = 6, height = 4, units = "in", dpi = 300)



##---------------------------------------------------------------------------##
## Boxplot for Percent Reads in Peaks
p4 <- ggplot(metaatac, aes(x = GenoSample, y = pct_reads_in_peaks, color = Genotype)) +
      geom_boxplot() +
      theme_classic() +
      theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
      scale_color_manual(values = c("#191919", "#BB0BF8", "#5DCA9F", "#EEBC10")) +
      ...
      ylim(0, 100) + 
      NULL
ggsave(filename = "NA_ATAC_PERCENT_READS_IN_PEAKS_PER_SAMPLE_BOXPLOT.pdf", plot = p4, width = 6, height = 4, units = "in", dpi = 300)
